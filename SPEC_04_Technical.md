...
├── SmallScaleInt/         # 第三方工具源（仅创作；gitignore）：含 Character Creator（§15）与 Fantasy kingdom Tileset 1.1.0；Tileset 的 Example scene/Scripts 用 Unity 6 API，与工程 2021.3 不兼容，由 Editor `FantasyTilesetExampleCompileGuard` 写入永不成立 defineConstraints 的 asmdef 停编；正式 Tile/Sprite 已复制到 Art/Maps/Environment
└── ConfigTables/          # 配置表统一根（见 §14）
    ├── Excel/             # 人配源表（.xlsx）；运行时不加载
    └── Csv/               # 程序读表（.csv）；打表产物；运行时唯一数据源
```

**Art vs Prefabs：** `Art/` 存源素材（图、Clip、Controller、贴图等）；游戏 Instantiate / Catalog 绑定只引用 `Prefabs/<模块>/`。本版 **不** 单独落地顶层 `Sprites/`；2D 图标统一落在 `Art/UI/`（及 `Art/Placeholder/` 过渡）。**运行时光标（UI-024）：** `Art/UI/Cursor.png` 由 `PlayerSettings.defaultCursor` 引用，**不**走 Prefab Instantiate。**例外（UI-016 / D-055 躯体/法阵）：** 源图 `Assets/Art/UI/Dig/{ArtAssetId}.png`（含 `MagicCircle_1`）；运行时副本 `Assets/Resources/UI/Dig/{ArtAssetId}`；`Resources.Load<Sprite>("UI/Dig/"+ArtAssetId)`；缺图按 `BodySlot` 回退同槽已有图。**例外（UI-021 / UI-025 / D-065 / D-071）：** 士兵技能图标须按 `SkillId` 运行时热加载，投放目录为 `Assets/Resources/UI/Skills/`（文件名 = `SkillId`，如 `Skill_01.png`）；`Resources.Load<Sprite>("UI/Skills/"+SkillId)`；缺图则空框仍显示技能名（布阵悬浮框）或空框图标（CombatSkillIcon）。**例外（UI-022 / UI-023 / UI-026）：** 主角装备 / 魔法书 / 商店待售商品图标：投放 `Assets/Resources/UI/Equipment/{IconAssetId}.png` 与 `Assets/Resources/UI/MagicBooks/{IconAssetId}.png`；表内 `IconAssetId` 仅文件名时由 View 加前缀加载；含 `/` 则按 Resources 相对路径直载。**例外（UI-031 / D-086）：** 关卡路线底图源图 `Assets/Art/UI/SubLevelMaps/{RouteMapAssetId}.png`；运行时副本 `Assets/Resources/UI/SubLevelMaps/{RouteMapAssetId}`；`Resources.Load<Sprite>("UI/SubLevelMaps/"+RouteMapAssetId)`；展示宽锁 **1450**、高按 Sprite 比例；导入建议 `maxTextureSize≥4096`、关 mipmap；**选项钉点**权威在每关 Prefab `Assets/Prefabs/Level/LevelRouteMap_{LevelId}.prefab`（子节点名=`GameplayOptionId`，`anchoredPosition`=卡片中心；底图左下原点、Y 向上）；运行时 `Resources.Load<GameObject>("Prefabs/Level/LevelRouteMap_"+LevelId)`（Ensure 同步副本 `Assets/Resources/Prefabs/Level/`）；缺钉 Warning、卡片仍于 `(0,0)`；无该 Prefab（或无 `RouteMapAssetId`）→ 旧 Stage 行布局。**地图模式 Icon 通关勾：** 源 `Assets/Art/UI/Icons/Checkmark.png`；运行时 `Assets/Resources/UI/Icons/Checkmark`；`Resources.Load<Sprite>("UI/Icons/Checkmark")`。**战斗指示器简画（UI-033 / D-089）：** 源 `Assets/Art/UI/Icons/{SilhouetteIconAssetId}.png`（及框体 `HPPK_UI_1`～`4`）；运行时副本 `Assets/Resources/UI/Icons/{Id}`；`Resources.Load<Sprite>("UI/Icons/"+Id)`；缺图空框。**地图流动水面（FlowingWater）：** 正式 shader/贴图/材质落在 `Assets/Art/Maps/Shaders/Water/`（Built-in；禁止运行时引用 `SmallScaleInt/` 或 URP include）；用法见 [§13](#13-资源编排与可扩展性)。**地图自动接边（MapAutoTile）：** Isometric Rule Tile 资产落在 `Assets/Art/Maps/RuleTiles/`；用法见 [§13](#13-资源编排与可扩展性)。**地图边缘迷雾（MapEdgeFog）：** 世界空间雾片挂各地图 Prefab（方案 A）；贴图可复用 `Art/Maps/Fogs/Fog_*.png`；非表型调参可 SO → `Assets/Settings/Maps/`；用法见 [§13](#13-资源编排与可扩展性)。角色烘焙细则见 [§15](#15-角色美术管线character-creator-烘焙整角)。

实际目录以工程为准；结构性变更记入 SPEC_00 Changelog。配置表路径与命名强制约定见 [§14](#14-配置表工程约定与打表工具)。角色美术路径与工具目录禁入见 [§15](#15-角色美术管线character-creator-烘焙整角)。

### English

Recommended tree as above. `Art/` holds source art (Characters per [§15](#15-角色美术管线character-creator-烘焙整角), Dig/Maps/Defend/UI/VFX/Audio, plus `Placeholder/` for Demo CSV `AssetPath`); runtime Instantiate uses `Prefabs/` only. Top-level `Sprites/` is **not** used this revision—2D icons live under `Art/UI/`. **Runtime pointer (UI-024):** `Art/UI/Cursor.png` is referenced by `PlayerSettings.defaultCursor`, **not** Prefab Instantiate. **Exception (UI-021 / UI-025 / D-065 / D-071):** soldier-skill icons load by `SkillId` at runtime from `Assets/Resources/UI/Skills/` (filename = `SkillId`; `Resources.Load<Sprite>("UI/Skills/"+SkillId)`; missing sprite still shows the skill name in the formation tooltip, or an empty-frame CombatSkillIcon). **Exception (UI-022 / UI-023 / UI-026):** protagonist equipment / MagicBook / shop-offer icons live under `Assets/Resources/UI/Equipment/{IconAssetId}.png` and `Assets/Resources/UI/MagicBooks/{IconAssetId}.png`; filename-only table ids get the folder prefix at load; ids containing `/` load as Resources-relative paths. **Exception (UI-031 / D-086):** level-route map source `Assets/Art/UI/SubLevelMaps/{RouteMapAssetId}.png`; runtime copy `Assets/Resources/UI/SubLevelMaps/{RouteMapAssetId}`; `Resources.Load<Sprite>("UI/SubLevelMaps/"+RouteMapAssetId)`; display width locked **1450**, height by Sprite aspect; import `maxTextureSize≥4096`, no mipmaps; **option pin authority** is per-level Prefab `Assets/Prefabs/Level/LevelRouteMap_{LevelId}.prefab` (child name=`GameplayOptionId`, `anchoredPosition`=card center; map bottom-left origin, Y up); runtime `Resources.Load<GameObject>("Prefabs/Level/LevelRouteMap_"+LevelId)` (Ensure copies `Assets/Resources/Prefabs/Level/`); missing pin → Warning, card at `(0,0)`; without that Prefab (or no `RouteMapAssetId`) → legacy Stage-row layout. **Map-mode cleared Checkmark:** art `Assets/Art/UI/Icons/Checkmark.png`; runtime `Assets/Resources/UI/Icons/Checkmark`; `Resources.Load<Sprite>("UI/Icons/Checkmark")`. **CombatIndicator silhouettes (UI-033 / D-089):** art `Assets/Art/UI/Icons/{SilhouetteIconAssetId}.png` (plus chrome `HPPK_UI_1`–`4`); runtime copy `Assets/Resources/UI/Icons/{Id}`; `Resources.Load<Sprite>("UI/Icons/"+Id)`; missing → empty frame. **Map FlowingWater:** canonical shaders/textures/mats under `Assets/Art/Maps/Shaders/Water/` (Built-in; no runtime `SmallScaleInt/` or URP includes); usage in [§13](#13-资源编排与可扩展性). **MapAutoTile:** Isometric Rule Tile assets under `Assets/Art/Maps/RuleTiles/`; usage in [§13](#13-资源编排与可扩展性). **MapEdgeFog:** world-space fog strips on each map Prefab (Approach A); art may reuse `Art/Maps/Fogs/Fog_*.png`; non-table tunables may use SO → `Assets/Settings/Maps/`; usage in [§13](#13-资源编排与可扩展性). Also includes `Materials/AllIn1/` (optional AllIn1 sprite-effect mats, §15.2), `Prefabs/Dig|Defend|Maps/...`, `SmallScaleInt/` vendor tool source (gitignore; Character Creator per §15 plus Fantasy kingdom Tileset 1.1.0 — Example scene/Scripts use Unity 6 APIs incompatible with project 2021.3 and are excluded from compile via Editor `FantasyTilesetExampleCompileGuard` writing an asmdef with an unsatisfied `defineConstraints`; canonical tiles/sprites live under `Art/Maps/Environment`), `ConfigTables/Excel/` + `ConfigTables/Csv/`. Record structural changes in SPEC_00 Changelog. Config-table path and naming rules: [§14](#14-配置表工程约定与打表工具). Character art paths and vendor-folder ban: [§15](#15-角色美术管线character-creator-烘焙整角).
...
**进档难度 Hub（UI-029 / D-081 方案 C）：** **权威源**独立 Prefab `Assets/Prefabs/Meta/InSaveShellPanel.prefab`；`MetaShellRoot` / Boot 仅嵌套其 **Prefab Instance**（禁止把进档壳整树 bake 进 `MetaShellRoot`）。菜单 `Ensure InSaveShellPanel`：补缺结构、把根上嵌套改为 Instance、**不得**用 `MetaShellRoot` 反向覆盖权威 Prefab、**不得**在已有 `ColumnsScroll`+`DescriptionText` 时拆毁手调难度栏。**新建进档** / 工具「关卡」/ 关卡结束回壳：默认中心面 `DifficultySelectHost`（`ColumnsScroll` 下等宽三栏普通/困难/地狱**同屏**，各约 1/3 视口）；悬停 Column 显示难度描述（Demo 写死）；**仅普通**点击 → `TryEnterLevel`（默认已解锁末项 LevelId）并打开 UI-031；困难/地狱可悬停、点击 Toast「还未制作」。**进入占用档**：跳过 Hub，直接 `TryEnterLevel` 开 UI-031；默认 LevelId=已解锁关中 `LevelRouteProgress` Cleared 选项所属 **StageNumber 最大** 的 LevelId（并列取解锁列表更后者；无通关 → 已解锁末项）；地图竖滑仍滚到最新已解锁前沿。三栏均使用作者指定难度贴图；`NormalColumn` / `HardColumn` / `HellColumn` 均保留各自 `Image.m_Sprite`，并统一 `Image.preserveAspect=true`，禁止回退为纯色无图背景。**不再**把 `LevelSelectPanel` 嵌于栏内作为主路径（Prefab 可保留、默认隐藏）。**无**中央 `MapHost` / 地图底图。关卡运行中 Hide Hub。左下商店/装备/魔法书/返回与右上工具锚点不变。issues：`.scratch/insave-difficulty-hub/`。

**士兵外观 From-Art（D-056 方案 B，WA-01 已编码）+ 职业区全覆盖（D-057，WA-02 已编码；v0.84.05 扩 PushMap）：** 扩展 `WarriorAppearancePrefabAssembler`：Art 就绪且缺 `Warriors/{AppearanceId}.prefab` 时从 Art 创建 Visual；已有只修结构。然后用 `CloneApp02FallbackAppearances.RefreshWarriorCatalogBindings` 刷新 Defend/UM Catalog（并集 Mode1+Mode2 `BodyAppearanceConfig` 已有 Prefab 的 AppearanceId），**禁止** `GenerateAll`。`EnsureFormationClassZones` / 菜单「Ensure Formation Class Zones on Maps」以 **Mode2** `Manufacture_ClassConfig` 为权威 ClassId（读 CSV；缺补/表外删；第二前/后排及 `_0` 缺区偏移见 §13；已有区保留世界 XZ；样例 HalfExtents 锁定 `(3.85, 2)`；父/子 localRotation=identity，废止 IsoTileYaw）。**PushMap：** 菜单 `Gravedigger2026/PushMap/Ensure Formation Class Zones on Sample Maps` 对 `PushMap_Demo_01/02/03` 同口径补区，锚点=`CameraFollowPath/WP_Start`（缺则 DigMapBounds.Center）。issues：`.scratch/mode2-warrior-art-bind/`。

**自动制造 Stage（规则已锁；AM-03～13 闭环 + D-054/D-055 + Step2 单槽套书；UI-016 尸骸雨方案 A）：** `GameplayType=AutoManufacture` → `AutoManufactureStageModule` + `AutoManufactureService` + `AutoFormationDeployService`；`GameplayConfigId` 忽略；循环选料/职业/基础属性（**造兵时不套书**；默认定种族）→ 授予 `DefaultSkillIds` → StaticStat/外观/命名 → `TempWarriorWarehouse` → **先 Clear `BattleFormation`** → flush→`WarriorPool`（收集本批 Id）→ 批次记录 →（crafted>0）UI-016：**StepA** 落下本批 `SourceItemIds` 躯体（`ArtAssetId`→`Resources/UI/Dig`；2D Physics 池化堆叠；按 `WarriorId` 标记）→ **StepB** 法阵 `MagicCircle_1` 闪红 + 谜底从法阵中心与每槽脉冲并行上升（每槽 1/6 + 峰值 `ApplyEquippedBookAtSlot` + 闪烁）+ 该兵堆中躯体错开飞入谜底视觉中心闪红消失（`AutoMfgBodyAbsorb*`）→ 脚底到 `LandY` 变兵定住 → **再**按最终 `PlacementOrder` 落入 `FormationClassZone`；不计 Spirit/Control；不写 SoulId；演出失败/`Exit` → `ApplyRemainingSlots` 兜底；**D-054** 批末 `Replace`；**D-055** 播完再 Advance + Mode2 UM `AutoOpenFormationOnce`；0 兵 Tips + 跳过演出。UI-016 Prefab `AutoManufacturePresentationRoot`：Overlay 背景 `Title_AutoManufacture_1` + Dim + `BookRow`；独立 2D 演出层（Overlay 1920x1080 Image 层 + 像素重力 + 可转 OBB SAT 堆叠；Prefab `AmBodyPartPiece` / `AmReviveSoldierPiece`）；中央 `SoldierScroll` **默认隐藏保留**（`AutoMfgUseLegacySoldierRow=1` 切回旧传送带+Camera+RT）。常量 `CombatConstantConfig` `AutoMfg*`。issues `.scratch/auto-mfg-body-rain/`；0 兵诊断 `.scratch/auto-mfg-craft-zero/`（`RunBatch` 结束 Console `Stock dump`，不改选料）。MetaShell 进档 Bind / 回档 Clear / 删档 Delete。`AmAssetBuilder.EnsurePresentationBackground` 外科补丁既有 Prefab（勿为加背景整表 Regen）。
...
**一键上阵（Mode2 布阵编辑器；D-074）：** `FormationEditorRoot_Mode2` 在 `CompleteButton` 左侧增加“一键上阵”按钮；点击后从当前地图收集 `FormationClassZone`，按士兵 `WarriorInstance.ClassId` 分组，并对**未上阵**的士兵进行区内随机放置（同一职业区内非重叠：候选点遍历+占用 Footprint 分离；无区/无空位则留池）；已上阵士兵不重置。实现复用 `FormationZoneSpiralSearch` 的区内安全判定与分离约束。**样例图须含区：** `Ground_*`（UM/Defend）、`PushMap_Demo_*`（推图 Prepare；Ensure 锚 `WP_Start`）、`SearchExtract_Demo_01`（搜打撤 Prepare）。**D-084：** 上述操作完成后须调用 `TacticalFormationLayoutService.EvaluateAndApply`（≥Min snap Pattern 槽位覆盖职业区；整阵拖拽；见 §3.18）。
...
**PushMap BOSS 通关与奖励钩子（方案 A，PM-07 + UI-017/018 扩展）：** `PushMapSessionService` 对 `IsBoss` 刷出行累计待击杀数；`TryNotifyBossKilled` 递减，归零 → `Ended` + `VictorySettled(StageExpReward)`；另计战斗耗时、击杀数、CaptureLoot 展示 ledger、**阵亡统计 `BattleCasualtyStats`**；叛变同步 `IsRebel`；无忠诚存活 → `RequestLevelFailure`。表现：`AddExperience` → **UI-017**（非立即 `_onVictoryAdvance`）；失败同弹结算。胜利 Continue → UI-018 → 结束关卡 + LevelSelect；失败「返回主界面」→ Abort + TitleMenu；「重新开始」→ Abort 后重进同 Option。占领仍当场入账 `CaptureLoot`。SearchExtract **复用**同一 UI-017 Factory。禁止运行时引用 `SmallScaleInt/`。
...
**离屏刷怪边缘提示（UI-034 / D-090 / 方案 A）：** 共享 `OffScreenSpawnHintRuntimeFactory` / View（可先 RuntimeFactory）；ScreenSpaceOverlay 参考 1920×1080；`sortingOrder` 略高于 CombatIndicator（例 70）；`raycastTarget=false`。判定：刷怪请求的 `basePos` + 战斗相机 `WorldToViewportPoint`；视口外钳到边缘（边距常量）；图标 `Resources/UI/Icons/EnemyAttack_1`；开场闪红/常亮/缩放/边距/尺寸 ← §9.20b `OffScreenSpawnHint*`。PushMap / SearchExtract StageController 在真实刷怪后 `TryShow`；PreparePreview 跳过；Hide 时 Clear。Defend **不接线**。issues `.scratch/offscreen-spawn-hint/`。
...
**InSave difficulty Hub (UI-029 / D-081 Approach C):** **Authoritative** standalone Prefab `Assets/Prefabs/Meta/InSaveShellPanel.prefab`; `MetaShellRoot` / Boot nest a **Prefab Instance** only (must not bake the full InSave tree into `MetaShellRoot`). Menu `Ensure InSaveShellPanel`: fill missing structure, replace baked nest with Instance, **must not** overwrite the authoritative Prefab from `MetaShellRoot`, **must not** destroy authored difficulty columns when `ColumnsScroll`+`DescriptionText` already exist. **Create enter** / Tools Level / LevelEnded: default center `DifficultySelectHost` — equal-width Normal/Hard/Hell **same-screen** under `ColumnsScroll` (~1/3 viewport each); hover Column shows difficulty description (Demo hardcoded); **Normal only** click → `TryEnterLevel` (default last unlocked LevelId) and open UI-031; Hard/Hell hover OK, click → Toast「还未制作」. **Enter occupied:** skip Hub; `TryEnterLevel` opens UI-031; default LevelId = unlocked level hosting Cleared `LevelRouteProgress` options with **max StageNumber** (tie → later in unlocked list; none Cleared → last unlocked); map still scrolls to latest unlocked frontier. All three columns must keep authored difficulty art; `NormalColumn` / `HardColumn` / `HellColumn` each retain their own `Image.m_Sprite`, and all use `Image.preserveAspect=true`; do not regress them to solid-color image backgrounds without sprites. **No longer** embeds `LevelSelectPanel` in-column as primary path (Prefab may remain, hidden). **No** central `MapHost` / map image. Hide Hub while Level running. Bottom-left Shop/Equipment/MagicBook/Back and top-right Tools anchors unchanged. issues: `.scratch/insave-difficulty-hub/`.

**Soldier From-Art visuals (D-056 Approach B, WA-01 coded) + full class-zone cover (D-057, WA-02 coded; v0.84.05 extends PushMap):** Extend `WarriorAppearancePrefabAssembler`: create Visual from Art when Prefab missing and Art ready; existing Prefabs keep layout-only fix. Refresh Defend/UM catalogs via `CloneApp02FallbackAppearances.RefreshWarriorCatalogBindings` (union Mode1+Mode2 `BodyAppearanceConfig` AppearanceIds that already have Prefabs); **do not** `GenerateAll`. `EnsureFormationClassZones` / menu "Ensure Formation Class Zones on Maps" uses **Mode2** `Manufacture_ClassConfig` as authoritative ClassId list (CSV; add missing / remove orphans; second front/back + `_0` offsets: §13; existing zones keep world XZ; sample HalfExtents locked to `(3.85, 2)`; parent/child localRotation=identity, IsoTileYaw dropped). **PushMap:** menu `Gravedigger2026/PushMap/Ensure Formation Class Zones on Sample Maps` syncs `PushMap_Demo_01/02/03` the same way, anchor=`CameraFollowPath/WP_Start` (else DigMapBounds.Center). Issues: `.scratch/mode2-warrior-art-bind/`.

**AutoManufacture stage (rules locked; AM-03–13 closed + D-054/D-055 + per-slot MagicBook; UI-016 body-rain Approach A):** `GameplayType=AutoManufacture` → `AutoManufactureStageModule` + `AutoManufactureService` + `AutoFormationDeployService`; ignore `GameplayConfigId`; pick/class/base (**no MagicBook at craft**; default race) → grant `DefaultSkillIds` → StaticStat/appearance/name → `TempWarriorWarehouse` → **Clear `BattleFormation` first** → flush→`WarriorPool` → batch record → (crafted>0) UI-016 **StepA** rain consumed `SourceItemIds` (`ArtAssetId`→`Resources/UI/Dig`; pooled 2D Physics pile tagged by `WarriorId`) → **StepB** `MagicCircle_1` flash + mystery rises from circle center in parallel with per-slot pulses (1/6 per slot + peak `ApplyEquippedBookAtSlot` + flash) + that soldier's pile pieces stagger-fly into mystery visual center with red flash (`AutoMfgBodyAbsorb*`) → morph Idle in place at `LandY` → **then** deploy by final `PlacementOrder` into `FormationClassZone`; ignore Spirit/Control; no SoulId; presentation fail/`Exit` → `ApplyRemainingSlots`; **D-054** batch `Replace`; **D-055** advance after play + Mode2 UM `AutoOpenFormationOnce`; 0 craft Tips + skip presentation. UI-016 Prefab `AutoManufacturePresentationRoot`: Overlay background `Title_AutoManufacture_1` + Dim + `BookRow`; separate 2D presentation layer (Overlay 1920x1080 Image + pixel-space gravity + rotatable OBB SAT pile; Prefabs `AmBodyPartPiece` / `AmReviveSoldierPiece`); center `SoldierScroll` **hidden by default** (`AutoMfgUseLegacySoldierRow=1` restores legacy conveyor+Camera+RT). Constants `CombatConstantConfig` `AutoMfg*`. issues `.scratch/auto-mfg-body-rain/`; zero-craft diagnostics `.scratch/auto-mfg-craft-zero/` (`RunBatch` Console `Stock dump`, pick rules unchanged). MetaShell Bind/Clear/Delete. `AmAssetBuilder.EnsurePresentationBackground` surgically patches existing Prefab (do not full Regen just to add background).
...
**One-click deploy (Mode2 formation editor; D-074):** `FormationEditorRoot_Mode2` adds a button left of `CompleteButton`; click collects current-map `FormationClassZone`, deploys **not-yet-deployed** soldiers by `ClassId` into matching zones (in-zone non-overlap; no zone/slot → leave in pool); already-deployed soldiers are not reset. Reuses `FormationZoneSpiralSearch`. **Sample maps must include zones:** `Ground_*` (UM/Defend), `PushMap_Demo_*` (PushMap Prepare; Ensure anchors `WP_Start`), `SearchExtract_Demo_01` (SearchExtract Prepare). **D-084:** after this, call `TacticalFormationLayoutService.EvaluateAndApply` (≥Min snap Pattern slots over class zones; whole-squad drag; §3.18).
...
**PushMap Boss clear & reward hooks (Approach A, PM-07 + UI-017/018 extended):** `PushMapSessionService` tracks pending Boss count; `TryNotifyBossKilled` → 0 → `Ended` + `VictorySettled(StageExpReward)`; also tracks combat time, kill count, CaptureLoot display ledger, **`BattleCasualtyStats`**; sync Rebel `IsRebel`; no living loyal → `RequestLevelFailure`. Presentation: `AddExperience` → **UI-017** (not immediate `_onVictoryAdvance`); failure shows the same panel. Win Continue → reward popup → end Level + LevelSelect; defeat Return to Title → Abort + TitleMenu; Restart → Abort then re-enter same Option. Capture still credits `CaptureLoot` immediately. SearchExtract **reuses** the same UI-017 factory. Do not runtime-reference `SmallScaleInt/`.
...
**OffScreenSpawnHint (UI-034 / D-090 / Approach A):** shared `OffScreenSpawnHintRuntimeFactory` / View (RuntimeFactory OK first); ScreenSpaceOverlay ref 1920×1080; `sortingOrder` slightly above CombatIndicator (e.g. 70); `raycastTarget=false`. Gate: spawn `basePos` + combat camera `WorldToViewportPoint`; clamp to edge when outside; icon `Resources/UI/Icons/EnemyAttack_1`; intro blink/hold/scale/margin/size ← §9.20b `OffScreenSpawnHint*`. PushMap / SearchExtract StageControllers `TryShow` after real spawns; skip PreparePreview; Clear on Hide. Defend **unwired**. issues `.scratch/offscreen-spawn-hint/`.
...
| LevelName | 关卡名称 | `string` | UI 显示名（UI-031 页签 Label + `Box/Title`）；同 LevelId 多行取**首个非空**；多行非空且不一致 → Warning 仍用首个；空=回退显示 `LevelId`。Demo 样例：`Level_01`→「第1关」、`Level_02`→「第2关」、`Level_03`→「第3关」 |
| DifficultyId | 难度ID | `string` | FK → [§9.1a `DifficultyConfig`](#91a-关卡难度表-difficultyconfig)；同 LevelId 多行须同值；空=不归属任何难度（不参与难度通关统计） |
...
| RouteMapAssetId | 路线底图资源ID | `string` | 可选；文件名无扩展名（例 `SubLevel_001`）；源 `Art/UI/SubLevelMaps/{Id}.png`；运行时 `Resources.Load<Sprite>("UI/SubLevelMaps/"+Id)`；同 LevelId 多行取**首个非空**；多行非空且不一致 → Warning 仍用首个；空=无底图意图（旧 Stage 行布局）；有值时 Ensure 菜单用以给 `LevelRouteMap_{LevelId}` 贴 Background |
| UnlockLevelId | 解锁 LevelId | `string` | **仅读 StageNumber=1**（其它 Stage 行忽略）。空=本 LevelId 默认解锁；值 ∈ SubLevel `GameplayOptionId` → 该选项已通关则解锁；值 ∉ SubLevel → 永不可解锁（正式入口）；加载时未知值 Warning |

```
LevelOperationConfig {
  LevelId: Id
  LevelName: string | ""          // UI display; first non-empty per LevelId; empty→fallback LevelId
  DifficultyId: Id | ""           // FK → DifficultyConfig; same across LevelId rows; empty=unowned
  StageNumber: int
  GameplayOptionId1..5: Id | ""   // FK → SubLevelConfig; empty ignored; max 5
  RouteMapAssetId: Id | ""        // per-Level map art id; first non-empty; pairs with LevelRouteMap_{LevelId}
  UnlockLevelId: Id | ""          // Stage1 only; empty=unlocked; known OptionId=prereq; unknown=never
}
```

`GameplayType` / `GameplayConfigId` **已迁出**至子关卡表（§9.31）。规则语义见 [SPEC_03 §3.9](SPEC_03_GameRules.md)（含 LevelId 门闩与难度通关）。

**Mode2 Demo 分叉（SE-03）：** `Level_01` 末战 Stage 同时挂 `Opt_L01_S5_PushMap` 与 `Opt_SE_Demo_01`；`Opt_L01_S4_UpgradeManufacture.UnlockNextOptionIds=Opt_L01_S5_PushMap|Opt_SE_Demo_01`（空 UnlockNext 仍=关卡胜利）。**Demo：** 现有 Level_01/02/03 行 `DifficultyId=Diff_Normal`。

#### 9.1a 关卡难度表 `DifficultyConfig`

**磁盘名：**
- **Excel：** `关卡_关卡难度表_Level_DifficultyConfig.xlsx`
- **CSV：** `Level_DifficultyConfig.csv`

| 字段 (EN) | 中文 | 类型（伪） | 说明 |
|-----------|------|------------|------|
| DifficultyId | 难度ID | `string` | 主键；样例 `Diff_Normal` / `Diff_Hard` / `Diff_Hell` |
| DisplayName | 显示名 | `string` | UI 栏标题 |
| UnlockRequireDifficultyId | 解锁所需难度ID | `string` | 空=**初始解锁**；值 ∈ 本表 DifficultyId → 等该难度已通关后解锁本行；值 ∉ 本表 → **不可解锁**（加载可 Warning；运行时当锁死） |
| Description | 文字介绍 | `string` | 悬停/说明文案；可空 |
| ClearReward | 难度通关奖励 | 见编码 | 可选；`ItemId;Count\|…`（同 CaptureLoot / SubLevel `Reward`；经 [§9.5a](#95a-道具汇总表-itemcatalogconfig)）；**首次**难度通关一次性发放；空=无奖 |

**`ClearReward` 编码（固定）：** 与 §9.5a / CaptureLoot 相同 — `ItemId;Count|ItemId;Count|...`；`ItemId` 可含下划线，按**最后一个分号**切数量。

```
DifficultyConfig {
  DifficultyId: Id
  DisplayName: string
  UnlockRequireDifficultyId: Id | ""   // empty=initial; known=after that difficulty cleared; unknown=never
  Description: string | ""
  ClearReward: "ItemId;Count|..." | ""
}
```
...
| ClassRestrict | 职业限定 | 见编码 | 可产出的 `ClassId` 多值；Mode2 双手交集定职业；空=该主要手配置错误，跳过该件试下一档（见 §3.15） |
| BaseClass | 基础职业 | `enum` / `string` | 中文 `战士\|射手\|法师\|刺客`（同 [§9.9b](#99b-职业配置表-classconfig) `BaseClass`）；**Dig HUD 主要手职业统计**按本列归桶；主要手行须填；非主要手可空→`Unspecified`；缺列→`Unspecified` |
...
BodyPartConfig {
  ...
  BaseClass: 战士 | 射手 | 法师 | 刺客 | empty  // Dig HUD + ClassConfig.BaseClass enum
...
- Dig HUD `WarehouseHudStats`：残骸 = `IsPrimaryHand≠1` 堆叠和；主要手按 `RaceId` / `BaseClass` 分桶（见 [SPEC_03 §3.10](SPEC_03_GameRules.md)）。
- Mode1 表可缺新列（加载缺省：`IsPrimaryHand=0`、`ClassRestrict` 空、`BaseClass=Unspecified`、`BodyPrimaryStat` 可空但 Mode2 行应填；`DisplayName` 空则 UI 回退 `BodyPartId`）。
- DigStageSummary（UI-011）：每条奖励为 `DigSummaryItemCell`（方图标 + 右下数量 + 下方名称）；躯体名=`DisplayName`（空则 `BodyPartId`），图标=`DigBodyArtLoader`；精魂/非躯体名=`Id`，图标=`ItemIconLoader.LoadFromCatalog`；`Body` **920×620** 竖向 `ScrollRect`（约三行可视，超出可滑）；`Content` 网格每行最多 **5**。
- 具体数值行 **TBD**。
...
| SilhouetteIconAssetId | 士兵简画图标 | `string` 或空 | UI-033 战斗指示器敌方格子简画；仅文件名；运行时 `Resources/UI/Icons/{SilhouetteIconAssetId}`；源图 `Art/UI/Icons/`；缺列/空 = 空框仍显示格子；**不**参与战斗公式 |
...
MonsterConfig {
  ...
  SilhouetteIconAssetId: Id | ""             // UI-033 CombatIndicator; Resources/UI/Icons/{Id}
}
```

**加载约定（`ConfigCsvRepository`）：** `MonsterType` / `AggroMode` / `AlertRadius` / `BodyRadius` / `PushCoefficient` / `RepulsionScale` / `FacingYawFlip` / `ActiveMoveMult` / `PassiveMoveMult` / `WalkToRunSeconds` 缺省如上；`RunSpeed` 缺列或空或 ≤0 存 0（运行时 `ResolveRunSpeed` 回退 `MoveSpeed`）；`WalkToRunSeconds < 0` → 加载失败；非法 `MonsterType` / 非法枚举或 `AlertRadius < 0` / `BodyRadius < 0` / `PushCoefficient < 0` / `RepulsionScale < 0` / 非法 `FacingYawFlip` 整表加载失败（§14.5）。`NormalAttackAnims` / `WalkAnims` / `RunAnims` 缺列或空 → 空串（表现层回退默认基名/状态名）；**不**因空池失败。`SilhouetteIconAssetId` 缺列/空 → 空串（UI-033 空框）；加载器另写内存 `TableOrder`（CSV 数据行出现序，0-based）供指示器同 `MonsterType` 稳定排序；**不**落盘为 CSV 列。Mode1 Excel 亦须同步空列以兼容共享加载器（[§14.7](#147-agent-配置表变更守则)）。`MonsterType` **不**替代 `PushMapSpawnConfig.IsBoss`。PushMap 与 Defend 共用本表。
...
| `SearchExtractHoldOrthoSizeMin` | 搜打撤守点Size下限 | `3` | HoldFraming 正交 Size 下限（再与 `CameraOrthoSizeMin` 取交） |
| `SearchExtractHoldOrthoSizeMax` | 搜打撤守点Size上限 | `8` | HoldFraming 正交 Size 上限（再与 `CameraOrthoSizeMax` 取交） |
| `SearchExtractHoldMaxPanRadius` | 守点中心偏移半径 | `8` | look-at 距当前 Objective 世界 XZ 最大半径；圈外忠诚兵允许出镜 |
| `SearchExtractHoldViewportPad` | 守点视口边垫 | `0.08` | 外框四边视口垫（0～0.5） |
| `SearchExtractHoldTopHudPad` | 守点顶 HUD 垫 | `0.12` | 外框顶边额外垫（CountdownBar / CombatIndicator） |
| `SearchExtractHoldInnerPad` | 守点内框垫 | `0.18` | 内框边垫；全体在内框内满滞留才允许收紧 |
| `SearchExtractHoldZoomInDelaySeconds` | 守点拉近滞留秒 | `0.8` | 内框滞留后才允许减小 Size / 回中 |
| `SearchExtractHoldSmoothTimeOut` | 守点拉远平滑秒 | `0.2` | Size/中心拉远（扩大）SmoothDamp |
| `SearchExtractHoldSmoothTimeIn` | 守点拉近平滑秒 | `0.55` | Size/中心拉近（收紧）SmoothDamp；须大于 Out |
| `SearchExtractHoldSampleInterval` | 守点采样间隔秒 | `0.2` | 视口 AABB 低频采样间隔 |
| `SearchExtractHoldObjectiveBias` | 守点中心偏向 | `0.65` | look-at = lerp(士兵中心, Objective, bias)；`1`=钉 Objective |
| `SearchExtractDecisionIdleDelaySeconds` | 决策停步延迟秒 | `1` | UI-032 弹出后忠诚兵延迟停步并 Idle（任意搜集点数） |
...
**SE-CAM-03 样例锁定（v0.84.28）：** 上表 `SearchExtractHold*` 与 v0.84.27 初值相同（Size `3`～`8`、半径 `8`、滞留 `0.8`、Out `0.2` / In `0.55`）。几何盒测菜单 `Gravedigger2026/SearchExtract/Run HoldFraming Solver Checks (SE-CAM-01)`。Play Mode 手验见 `.scratch/search-extract-hold-camera/issues/03-tune-handcheck.md`。**未**改规则语义。
...
| `AutoMfgBodyDropIntervalSeconds` | 自动制造尸骸落下间隔 | `0.3` | UI-016 StepA：每批次掉落间隔秒 |
| `AutoMfgDropCountMin` | 自动制造尸骸每批最少 | `3` | StepA 每间隔随机落下件数下限 |
| `AutoMfgDropCountMax` | 自动制造尸骸每批最多 | `5` | StepA 每间隔随机落下件数上限；总数仍=本批 SourceItemIds |
| `AutoMfgGravityScale` | 自动制造尸骸重力 | `2` | Overlay 像素物理重力倍率（981 px/s² × 本值） |
| `AutoMfgBounciness` | 自动制造尸骸弹性 | `0.15` | 像素物理落地/互撞弱弹 |
| `AutoMfgFriction` | 自动制造尸骸摩擦 | `0.4` | 像素物理水平阻尼 + 角速度阻尼 |
| `AutoMfgColliderInset` | 自动制造尸骸碰撞内缩 | `0.72` | OBB 边长相对 Sprite 适配尺寸的比例（<1 使斜角更紧） |
| `AutoMfgSpawnAngleMaxDeg` | 自动制造尸骸落下转角 | `50` | 落下随机 Z 转角绝对值上限（度）；并作初始角速度尺度 |
| `AutoMfgSpawnJitterXPx` | 自动制造尸骸水平微扰 | `80` | 落点相对屏幕中心 X 微扰（参考 1920 宽） |
| `AutoMfgMagicCircleFlashHz` | 自动制造法阵闪红频率 | `8` | StepB 法阵红色闪烁 Hz |
| `AutoMfgMagicCircleYPx` | 自动制造法阵本地 Y | `-50` | `MagicCircle` 相对 `AmBodyRainLayer` 的 anchoredPosition.y |
| `AutoMfgBodyRainLayerYPx` | 自动制造尸骸雨层本地 Y | `-340` | `AmBodyRainLayer` 相对 PresentationRoot 的 anchoredPosition.y |
| `AutoMfgReviveSpawnOffsetYPx` | 自动制造复活出生上偏移 | `400` | **Demo 不再用作出生 Y**（键保留）；出生=法阵中心，终点=`AutoMfgSoldierLandYPx` |
| `AutoMfgPileFloorYPx` | 自动制造尸骸堆地板 Y | `-300` | 2D 演出层堆地板相对 `AmBodyRainLayer` Y（1920×1080） |
| `AutoMfgSoldierLandYPx` | 自动制造士兵落地 Y | `-410` | 复活士兵脚底落地相对 Layer Y（法阵下方） |
| `AutoMfgBodyMaxEdgePx` | 自动制造躯体适配边长 | `49` | StepA 躯体 Image 最长边像素（约原 140 的 35%） |
| `AutoMfgSoldierMaxEdgePx` | 自动制造士兵固定边长 | `64` | StepB 问号/士兵 Image 最长边（再 × VisualScale；底部 pivot） |
| `AutoMfgSoldierVisualScale` | 自动制造士兵视觉缩放 | `8` | 复活件 `localScale`（XYZ） |
| `AutoMfgSoldierPitchFactor` | 自动制造士兵行间距系数 | `0.5` | StepB 行间距 = MaxEdge×VisualScale×本值（样例 **256**） |
| `AutoMfgSoldierShadowWidthPx` | 自动制造士兵影子宽 | `20` | 复活件脚下椭圆影子宽（参考像素 / sizeDelta；Demo 隐影时仍保留） |
| `AutoMfgSoldierShadowHeightPx` | 自动制造士兵影子高 | `8` | 复活件脚下椭圆影子高（参考像素 / sizeDelta；Demo 隐影时仍保留） |
| `AutoMfgSoldierShadowAlpha` | 自动制造士兵影子透明度 | `0` | 脚下影子黑色 Alpha（0～1；**0**=Demo 隐藏） |
| `AutoMfgSoldierShadowOffsetYPx` | 自动制造士兵影子相对 Y | `-32` | 影子相对士兵脚底（底部 pivot）的本地 Y |
| `AutoMfgBodyAngleMaxDeg` | 自动制造躯体绝对转角上限 | `270` | 躯体 Z 转角夹紧 ±本值；触限清角速度 |
| `AutoMfgBodyAbsorbStaggerSeconds` | 自动制造躯体吸入起飞间隔 | `0.06` | StepB 同兵躯体件错开起飞间隔（秒；随 speed 缩短） |
| `AutoMfgBodyAbsorbFlySeconds` | 自动制造躯体吸入飞入时长 | `0.35` | StepB 单件飞入谜底视觉中心时长（秒；随 speed 缩短） |
| `AutoMfgBodyAbsorbFlashHz` | 自动制造躯体吸入闪红频率 | `8` | StepB 飞入中 Image 白↔红闪烁 Hz |
| `AutoMfgUseLegacySoldierRow` | 自动制造旧士兵行开关 | `0` | `1`=显示中央传送带旧流程；`0`=尸骸雨默认 |
...
| `ChaseStuckRetargetSeconds` | 追击卡住换目标秒数 | `1` | 追击未进距且位移不足达本秒 → 强制换目标 |
| `ChaseStuckRetargetCooldownSeconds` | 追击卡住换目标冷却 | `1` | 强制换目标后冷却，防 A↔B 甩 |
...
规则语义：被 `MonsterConfig.Skills` 段内 SkillId 引用（**不**走 `SkillConfig`）。PushMap / SearchExtract Demo D-074（方案 B 共享 `IMonsterDeathSkillHost`）首版：`MonsterSelfReviveOnDeath`。
...
- **样例地图：** `Assets/Prefabs/Maps/PushMap_Demo_01.prefab`（`MapId=PushMap_Demo_01`）；由 Editor 菜单 `Gravedigger2026/PushMap/Ensure Sample Map Prefab` 从 `Ground_01` 复制并挂齐标记；**不**改写 Dig/Defend 共用的 `Ground_*`。Mode2 关卡 2/3 另用作者图 `PushMap_Demo_02` / `PushMap_Demo_03`（`PushMapGameplayConfig`：`PushMap_02`/`PushMap_03`）。**职业区（D-057/D-074）：** 三张样例图须含 Mode2 全量 `FormationClassZone`；菜单 `Gravedigger2026/PushMap/Ensure Formation Class Zones on Sample Maps` 幂等补区，锚点=`CameraFollowPath/WP_Start`（缺则 DigMapBounds.Center）；`Ensure Sample Map Prefab` 亦会 Ensure 职业区
...
- **表现：** 订阅胜负 → 入账 Exp（仅胜）→ 显示 UI-017（含 `BattleCasualtyStats`）；胜利 Continue → UI-018 → 结束关卡 + LevelSelect；失败「返回主界面」→ Abort + TitleMenu，「重新开始」→ Abort + 重进同 Option；**延迟** driver 回调至按钮
...
session.BuildCasualtyStats() → BattleCasualtyStats
event VictorySettled(stageExp)
event LevelFailureRequested
CombatElapsedSeconds / MonstersKilled / CaptureLootLedger / IsVictory
PushMapBattleSettlementView.ShowVictory / ShowDefeat
PushMapRewardPopupView
Meta: onReturnTitle / onRestartOption
```
...
| LevelName | 关卡名称 | `string` | UI display name (UI-031 tab Label + `Box/Title`); first non-empty among LevelId rows; conflicting non-empty → Warning, still use first; empty → fallback to `LevelId`. Demo samples: `Level_01`→「第1关」, `Level_02`→「第2关」, `Level_03`→「第3关」 |
| DifficultyId | 难度ID | `string` | FK → [§9.1a `DifficultyConfig`](#91a-difficultyconfig); same value across LevelId rows; empty → not owned by any difficulty |
...
| RouteMapAssetId | 路线底图资源ID | `string` | Optional; filename without ext (e.g. `SubLevel_001`); source `Art/UI/SubLevelMaps/{Id}.png`; runtime `Resources.Load<Sprite>("UI/SubLevelMaps/"+Id)`; first non-empty among LevelId rows; conflicting non-empty → Warning, still use first; empty → no map intent / legacy Stage-row layout; when set, Ensure menu uses it to paint Background on `LevelRouteMap_{LevelId}` |
| UnlockLevelId | 解锁 LevelId | `string` | **StageNumber=1 only** (ignore other Stage rows). Empty → default unlocked; value in SubLevel `GameplayOptionId` → unlock when cleared; unknown → never unlockable (formal); load Warning on unknown |

`GameplayType` / `GameplayConfigId` moved to SubLevel (§9.31). Rules: [SPEC_03 §3.9](SPEC_03_GameRules.md) (LevelId gate + difficulty clear).

**Mode2 Demo fork (SE-03):** `Level_01` last-combat Stage mounts `Opt_L01_S5_PushMap` and `Opt_SE_Demo_01`; `Opt_L01_S4_UpgradeManufacture.UnlockNextOptionIds=Opt_L01_S5_PushMap|Opt_SE_Demo_01` (empty UnlockNext still = level victory). **Demo:** Level_01/02/03 rows use `DifficultyId=Diff_Normal`.

#### 9.1a DifficultyConfig
...
| Field (EN) | ZH | Type (pseudo) | Notes |
|------------|-----|---------------|-------|
| DifficultyId | 难度ID | `string` | PK; samples `Diff_Normal` / `Diff_Hard` / `Diff_Hell` |
| DisplayName | 显示名 | `string` | UI column title |
| UnlockRequireDifficultyId | 解锁所需难度ID | `string` | Empty = **initially unlocked**; value in this table → unlock after that difficulty clears; missing Id → **never unlockable** (load may Warning; treat as locked) |
| Description | 文字介绍 | `string` | Hover / help copy; may be empty |
| ClearReward | 难度通关奖励 | encoding | Optional; `ItemId;Count\|…` (same as CaptureLoot / SubLevel `Reward`; via [§9.5a](#95a-itemcatalogconfig)); granted **once** on first difficulty-clear; empty = none |

**`ClearReward` encoding (fixed):** same as §9.5a / CaptureLoot — `ItemId;Count|ItemId;Count|...`; split quantity at the **last semicolon**.

```
DifficultyConfig {
  DifficultyId: Id
  DisplayName: string
  UnlockRequireDifficultyId: Id | ""
  Description: string | ""
  ClearReward: "ItemId;Count|..." | ""
}
```
...
| ClassRestrict | 职业限定 | encoding | Multi-`ClassId` for Mode2 class pool; empty PrimaryHand = skip that part, try next (§3.15) |
| BaseClass | 基础职业 | `enum` / `string` | ZH `战士\|射手\|法师\|刺客` (same as [§9.9b](#99b-职业配置表-classconfig) `BaseClass`); Dig HUD primary-hand class buckets; required on primary-hand rows; empty/`Unspecified` OK for non-primary; missing column → `Unspecified` |
...
BodyPartConfig {
  ...
  BaseClass: 战士 | 射手 | 法师 | 刺客 | empty
  BodyPrimaryStat: Strength | Agility | Intelligence
}
```

**Resolve:** `Base(S) = Σ` filled parts' `StatBonus(S)` (missing dim **0**). Mode2 pick/class rules: [SPEC_03 §3.15](SPEC_03_GameRules.md). Warehouse stacks by `BodyPartId`; overflow uses row `AutoConvert`. Dig HUD `WarehouseHudStats`: wreck = sum where `IsPrimaryHand≠1`; primary hands bucket by `RaceId` / `BaseClass` ([SPEC_03 §3.10](SPEC_03_GameRules.md)). Mode1 may omit new columns (defaults: `IsPrimaryHand=0`, empty `ClassRestrict`, `BaseClass=Unspecified`, empty `DisplayName` → UI falls back to `BodyPartId`). DigStageSummary (UI-011): each reward is `DigSummaryItemCell` (square icon + bottom-right qty + name below); body name=`DisplayName` (empty → `BodyPartId`), icon=`DigBodyArtLoader`; Spirit/non-body name=`Id`, icon=`ItemIconLoader.LoadFromCatalog`; `Body` **920×620** vertical `ScrollRect` (~3 rows visible; overflow scrolls); `Content` grid max **5** per row. Concrete values **TBD**.
...
| SilhouetteIconAssetId | 士兵简画图标 | `string` or empty | UI-033 CombatIndicator enemy-slot silhouette; filename only; runtime `Resources/UI/Icons/{SilhouetteIconAssetId}`; art `Art/UI/Icons/`; missing/empty → empty frame, slot still shown; **not** used in combat math |
...
| `SearchExtractHoldOrthoSizeMin` | 搜打撤守点Size下限 | `3` | HoldFraming ortho Size floor (intersect with `CameraOrthoSizeMin`) |
| `SearchExtractHoldOrthoSizeMax` | 搜打撤守点Size上限 | `8` | HoldFraming ortho Size ceiling (intersect with `CameraOrthoSizeMax`) |
| `SearchExtractHoldMaxPanRadius` | 守点中心偏移半径 | `8` | Max look-at distance from current Objective world XZ; loyals outside may leave frame |
| `SearchExtractHoldViewportPad` | 守点视口边垫 | `0.08` | Outer viewport pad on four sides (0–0.5) |
| `SearchExtractHoldTopHudPad` | 守点顶 HUD 垫 | `0.12` | Extra top pad for CountdownBar / CombatIndicator |
| `SearchExtractHoldInnerPad` | 守点内框垫 | `0.18` | Inner pad; tighten only after all stay inside for dwell |
| `SearchExtractHoldZoomInDelaySeconds` | 守点拉近滞留秒 | `0.8` | Dwell before allowing Size shrink / recenter |
| `SearchExtractHoldSmoothTimeOut` | 守点拉远平滑秒 | `0.2` | SmoothDamp when expanding Size / pan-out |
| `SearchExtractHoldSmoothTimeIn` | 守点拉近平滑秒 | `0.55` | SmoothDamp when tightening; must be > Out |
| `SearchExtractHoldSampleInterval` | 守点采样间隔秒 | `0.2` | Low-frequency viewport AABB sample interval |
| `SearchExtractHoldObjectiveBias` | 守点中心偏向 | `0.65` | look-at = lerp(soldier center, Objective, bias); `1` = pin Objective |
| `SearchExtractDecisionIdleDelaySeconds` | 决策停步延迟秒 | `1` | After UI-032 show, delay then pause MassMove + Idle (any gather count) |
...
**SE-CAM-03 sample lock (v0.84.28):** `SearchExtractHold*` values match v0.84.27 initials (Size `3`–`8`, radius `8`, dwell `0.8`, Out `0.2` / In `0.55`). Geometric checks: menu `Gravedigger2026/SearchExtract/Run HoldFraming Solver Checks (SE-CAM-01)`. Play Mode handcheck: `.scratch/search-extract-hold-camera/issues/03-tune-handcheck.md`. Rule semantics **unchanged**.
...
| `AutoMfgBodyDropIntervalSeconds` | 自动制造尸骸落下间隔 | `0.3` | UI-016 StepA drop interval seconds |
| `AutoMfgDropCountMin` | 自动制造尸骸每批最少 | `3` | StepA random drop count min per interval |
| `AutoMfgDropCountMax` | 自动制造尸骸每批最多 | `5` | StepA random drop count max; total still = batch SourceItemIds |
| `AutoMfgGravityScale` | 自动制造尸骸重力 | `2` | Overlay pixel-physics gravity scale (981 px/s² × this) |
| `AutoMfgBounciness` | 自动制造尸骸弹性 | `0.15` | Pixel-physics soft bounce on floor / piece hits |
| `AutoMfgFriction` | 自动制造尸骸摩擦 | `0.4` | Pixel-physics horizontal + angular damping |
| `AutoMfgColliderInset` | 自动制造尸骸碰撞内缩 | `0.72` | OBB edge vs fitted sprite size (<1 lets slanted pieces nest) |
| `AutoMfgSpawnAngleMaxDeg` | 自动制造尸骸落下转角 | `50` | Max abs spawn Z yaw (deg); also scales initial angular velocity |
| `AutoMfgSpawnJitterXPx` | 自动制造尸骸水平微扰 | `80` | Spawn X jitter vs screen center (ref 1920) |
| `AutoMfgMagicCircleFlashHz` | 自动制造法阵闪红频率 | `8` | StepB circle red flash Hz |
| `AutoMfgMagicCircleYPx` | 自动制造法阵本地 Y | `-50` | `MagicCircle` anchoredPosition.y vs `AmBodyRainLayer` |
| `AutoMfgBodyRainLayerYPx` | 自动制造尸骸雨层本地 Y | `-340` | `AmBodyRainLayer` anchoredPosition.y vs PresentationRoot |
| `AutoMfgReviveSpawnOffsetYPx` | 自动制造复活出生上偏移 | `400` | **Not Demo spawn Y** (key retained); spawn=circle center, end=`AutoMfgSoldierLandYPx` |
| `AutoMfgPileFloorYPx` | 自动制造尸骸堆地板 Y | `-300` | 2D pile floor Y vs `AmBodyRainLayer` (1920×1080) |
| `AutoMfgSoldierLandYPx` | 自动制造士兵落地 Y | `-410` | Revived soldier feet land Y vs Layer (below circle) |
| `AutoMfgBodyMaxEdgePx` | 自动制造躯体适配边长 | `49` | StepA body Image max edge px (~35% of prior 140) |
| `AutoMfgSoldierMaxEdgePx` | 自动制造士兵固定边长 | `64` | StepB mystery/soldier Image max edge (then × VisualScale; bottom pivot) |
| `AutoMfgSoldierVisualScale` | 自动制造士兵视觉缩放 | `8` | Revive piece `localScale` (XYZ) |
| `AutoMfgSoldierPitchFactor` | 自动制造士兵行间距系数 | `0.5` | StepB row pitch = MaxEdge×VisualScale×this (sample **256**) |
| `AutoMfgSoldierShadowWidthPx` | 自动制造士兵影子宽 | `20` | Oval foot-shadow width (ref px / sizeDelta; retained while Demo-hidden) |
| `AutoMfgSoldierShadowHeightPx` | 自动制造士兵影子高 | `8` | Oval foot-shadow height (ref px / sizeDelta; retained while Demo-hidden) |
| `AutoMfgSoldierShadowAlpha` | 自动制造士兵影子透明度 | `0` | Foot-shadow black Alpha (0–1; **0**=Demo hide) |
| `AutoMfgSoldierShadowOffsetYPx` | 自动制造士兵影子相对 Y | `-32` | Shadow local Y vs soldier feet (bottom pivot) |
| `AutoMfgBodyAngleMaxDeg` | 自动制造躯体绝对转角上限 | `270` | Clamp body Z angle ±this; zero ω at limit |
| `AutoMfgBodyAbsorbStaggerSeconds` | 自动制造躯体吸入起飞间隔 | `0.06` | StepB per-soldier body stagger launch interval (s; scales with speed) |
| `AutoMfgBodyAbsorbFlySeconds` | 自动制造躯体吸入飞入时长 | `0.35` | StepB single-piece fly to mystery visual center (s; scales with speed) |
| `AutoMfgBodyAbsorbFlashHz` | 自动制造躯体吸入闪红频率 | `8` | StepB in-flight Image white↔red flash Hz |
| `AutoMfgUseLegacySoldierRow` | 自动制造旧士兵行开关 | `0` | `1`=legacy conveyor; `0`=body-rain default |
...
| `ChaseStuckRetargetSeconds` | 追击卡住换目标秒数 | `1` | Chase-stuck force-retarget window |
| `ChaseStuckRetargetCooldownSeconds` | 追击卡住换目标冷却 | `1` | Cooldown after forced retarget |
...
- **Sample map:** `Assets/Prefabs/Maps/PushMap_Demo_01.prefab` (`MapId=PushMap_Demo_01`); Editor menu `Gravedigger2026/PushMap/Ensure Sample Map Prefab` copies `Ground_01` and attaches markers; **do not** rewrite Dig/Defend shared `Ground_*`. Mode2 Level 2/3 also use author maps `PushMap_Demo_02` / `PushMap_Demo_03` (`PushMapGameplayConfig`: `PushMap_02`/`PushMap_03`). **Class zones (D-057/D-074):** all three sample maps must include Mode2-full `FormationClassZone`; menu `Gravedigger2026/PushMap/Ensure Formation Class Zones on Sample Maps` upserts zones anchored at `CameraFollowPath/WP_Start` (else DigMapBounds.Center); `Ensure Sample Map Prefab` also Ensures class zones
...
- **Presentation:** outcome → Exp (win) → UI-017 (incl. `BattleCasualtyStats`); win Continue → UI-018 → end Level + LevelSelect; defeat Return to Title → Abort + TitleMenu, Restart → Abort + re-enter same Option; defer driver callbacks to buttons
...
#### 9.29 BGM 配置表 `BgmConfig`

规则语义：[SPEC_03 §3.4](SPEC_03_GameRules.md) 背景音乐。一行 = 某一 `Context` 曲池中的一首候选曲。同 `Context` 多行按 `Weight` 加权随机选 1 首播放。

**磁盘名：**
- **Excel：** `音频_BGM配置表_Audio_BgmConfig.xlsx`
- **CSV：** `Audio_BgmConfig.csv`

| Field (EN) | ZH | Type (pseudo) | Notes |
|------------|-----|---------------|-------|
| BgmId | BGM行ID | `string` | PK |
| Context | 情境 | enum | `Title` \| `Dig` \| `Combat` |
| ClipId | 音频资源ID | `string` | 与 `BgmClipCatalog` 键一致；建议=文件名无扩展名（如 `BGM_Title_Graveglass Gate`）；源文件 `Assets/Art/Audio/BGM/` |
| Loop | 是否循环 | `0`\|`1` | 缺省 / 空 = `1`（循环）；`0` = 只播一遍 |
| Weight | 随机权重 | `int` ≥0 | 同 Context 加权；`0` 剔除；缺省 / 空 = `1` |
| Volume | 音量 | `float` 0～1 | 缺省 / 空 = `1` |

**加载约定（`ConfigCsvRepository`）：** Mode1/Mode2 均加载；非法 `Context` / 空 `BgmId`/`ClipId` / `Weight<0` / `Volume` 越界 → 整表失败。`Weight=0` 行保留但不参与随机。

**运行时：** `BgmService` + `BgmClipCatalog`（`Assets/Settings/Audio/`）；Clip **不**走 `Resources`；挂点见 [§6](#6-demo-实现边界) Meta 壳 / Dig Enter·Exit / Defend·PushMap `PhaseChanged`。

**Demo 样例：** 覆盖 `Art/Audio/BGM` 现有 9 首（Title×3 / Dig×1 / Combat×5）；`Loop=1`、`Weight=1`、`Volume=1`。

```
BgmConfig {
  BgmId: Id
  Context: Title | Dig | Combat
  ClipId: string
  Loop: 0 | 1
  Weight: int
  Volume: float
...
| 字段 (EN) | 中文 | 类型（伪） | 说明 |
|-----------|------|------------|------|
| GameplayOptionId | 玩法选项ID | `string` | 主键；**不得跨 LevelId 复用** |
...
| IconAssetId | 玩法图标 | `string` | Demo：`Resources/UI/Levels/{IconAssetId}`；缺图占位；**地图钉点**用 |
| IconAssetId2 | Tips图标2 | `string` | 仅悬停 Tips 中部大图；`Resources/UI/Levels/{Id}`（缺则试 `UI/Icons/`）；**空=不显示** |
| TipMessages | 提示消息 | `string` | 仅 Dig Tips；编码 `MsgType;StockScale|…`（最多 3 段）；MsgType 封闭枚举 Spirit/Wreck/Warrior/Archer/Assassin/Mage/Humans/Elves/Orcs/AllRaces；StockScale∈[-3,-1]∪[1,3]（正=Up×N，负=Down×|N|）；类型名←`LocalizedDescription` Key=`TipMsg_{MsgType}`；非法段跳过+Warning；空=无消息区 |
...
SubLevelConfig {
  ...
  IconAssetId: Id                   // map pin
  IconAssetId2: Id | ""            // Tips middle art; empty=hide
  TipMessages: "MsgType;Scale|..." | ""  // Dig Tips; max 3
  Title: string
  Description: string
  Reward: "ItemId;Count|..." | ""
...
**悬停 Tips 布局（UI-031 / `OptionHoverTips` Prefab）：** 权威资产 `Assets/Prefabs/Level/OptionHoverTips.prefab`；以 **Prefab 实例**嵌套于 `LevelRouteSelectRoot/Box`；Editor `Gravedigger2026/Level/Ensure OptionHoverTips Prefab (UI-031)` 生成/刷新，并同步运行时副本 `Assets/Resources/Prefabs/Level/OptionHoverTips.prefab`；`Ensure LevelRouteSelectRoot` 缺则 `InstantiatePrefab` 嵌套。运行时 `LevelRouteSelectView` **仅**用 SerializeField 驱动 `Show`/`Hide`；**禁止**运行时 `BuildHierarchy` 拼装（`BuildHierarchy` 仅 Editor 建资产）。按 `GameplayType`——**Dig**：Title + `TipMessages`（≤3：类型名/图标/`Up_1`·`Down_1` 尺度箭头）+ Description；**Shop / AutoManufacture / UpgradeManufacture**：Title + `IconAssetId2`（空隐藏）+ Description；**PushMap / SearchExtract / Defend**：Title + `IconAssetId2`（空隐藏）+ `Reward` 图标行（≤3，经 §9.5a）+ Description；Description 空则隐藏。地图钉点仍只用 `IconAssetId`。运行时图标源 `Art/UI/Icons/` → `Resources/UI/Icons/`（含 Up/Down 与 TipMsg 图标）。 **UI-034：** `EnemyAttack_1` 同路径（源 `Art/UI/Icons/EnemyAttack_1` → `Resources/UI/Icons/EnemyAttack_1`）。
...
**Demo 样例：** `SearchExtract_01`；`MapId=SearchExtract_Lv1_01`（关卡 1 正式图；另保留 `SearchExtract_Demo_01` 作 SE-02 方案 B 参考副本，**不**改写 PushMap 原图；图须含 Mode2 全量 `FormationClassZone` 供 Prepare「一键上阵」）；`GatherCountdownSeconds=80`；`StageExpReward=0`。**运行时绑定：** `SearchExtractStageController` 经 `DefendPrefabCatalog.TryGetMap(MapId)` 取 Prefab，**不**扫文件夹；`DefendPrefabCatalog.Maps` 与 `DefendAssetBuilder.CatalogExtraMapIds` 须覆盖玩法表引用的全部 `SearchExtract_*` MapId（含 `SearchExtract_Demo_01`、`SearchExtract_Lv1_01`、`SearchExtract_Lv2_01`）。
...
| WaveIndex | 配方序号 | `int` | ≥1；同点内唯一；复合键 = ConfigId+Order+WaveIndex |
| FirstWaveDelaySeconds | 首刷前置秒 | `float` | ≥0；自**该点首次激活**起；到期刷本行一次；各行可不同 |
| WaveIntervalSeconds | 重复间隔秒 | `float` | ≥0；本行**首刷之后**再计时；到期再刷本行一次；`RepeatSpawnCount=0` 时可填 0（忽略） |
| RepeatSpawnCount | 重复刷次数 | `int` | ≥0；首刷后再重复几次；总刷次数=`1+RepeatSpawnCount`；`>0` 时须 `WaveIntervalSeconds>0` |
| SpawnPointId | 刷怪点ID | `string` | FK 地图 `SpawnPoint`；缺标记 → 该次跳过 + Warning |
...
**激活规则：** 点首次激活后，该点**所有行并行**独立计时：自激活起 `FirstWaveDelaySeconds` → **首刷**本行；若 `RepeatSpawnCount>0`，则每次刷出后等待 `WaveIntervalSeconds` 再刷同一行，直至重复次数用尽。同点各行 Delay/Interval/Repeat **不必相同**。单点胜利瞬间停刷（不再首刷、不再重复）。**无** `WaveCount` / `ClockDirection` / `SpawnDirection`。

**Demo 样例（v0.84.00）：** `SearchExtract_01` 点 1 共 4 行（`WaveIndex` 1..4）；`FirstDelay`=5/10/15/20、`Interval`=0、`RepeatSpawnCount`=0（约 5/10/15/20s 各刷一次，无重复）；每行 `Monster_01`×10；`SpawnPointId`=`SP_01`～`SP_04`（须在当前 `MapId` 图上存在，现为 `SearchExtract_Lv1_01`；缺标记 → 该次跳过 + Warning）。

**开战镜头（v0.83.93 / HoldFraming v0.84.27 / 样例锁定 v0.84.28 / Continue Size 恢复 v0.84.43 / 轨 Bake 刷新 v0.84.45）：** 复用 `PushMapCameraFollowController`。**进圈前 / Continue 后接近** = 跟随地图 `CameraFollowPath` 与忠诚士兵投影（同 PushMap Combat；`CameraFollowDeadzone` / `CameraFollowSmoothTime`）。**点激活～GatherCountdown** = **HoldFraming**（`SearchExtractHoldFramingSolver` 视口包围+迟滞；常量 `SearchExtractHold*` ← §9.20b，SE-CAM-03 锁定初值）；UI-032 冻结；**Continue** → `ClearHoldFraming`：**强制 Auto**、look-at **不 Snap** 回轨，`orthographicSize` 以 `CameraFollowSmoothTime` SmoothDamp 恢复为 `PushMapCameraOrthoSize`（无新常量 Key；滚轮改 Size 取消本次恢复）。**开战**对 `CameraFollowPath` **总是** `TryBake` 一次（防 Prefab 路点已改但 `_bakedPoints` 过期；样例 `SearchExtract_Lv2_01` WP_Start→WP_S1→WP_End≈Obj1→Obj2）。PushMap **不**调用 Hold API。
...
规则语义：通用 UI / Tips 文案集中存放，便于后续多语言切换。Dig HUD Warehouse Hover Tips 等引用本表 `TextKey`（[SPEC_03 §3.10](SPEC_03_GameRules.md)）。

**磁盘名：**
- **Excel：** `通用_多语言描述表_Common_LocalizedDescriptionConfig.xlsx`
- **CSV：** `Common_LocalizedDescriptionConfig.csv`

| 字段 (EN) | 中文 | 类型（伪） | 说明 |
|-----------|------|------------|------|
| TextKey | 文案键 | `string` | 主键；代码/SPEC 引用（如 `DigWarehouseHoverTips`） |
| TextZh | 中文文案 | `string` | Demo **权威展示串**；运行时 `TryGetLocalizedText` 只读本列 |
| TextEn | 英文文案 | `string` | 预留；空允许；完整 i18n 切换后置 |
| Comment | 备注 | `string` | 策划说明；不参与逻辑 |

```
LocalizedDescriptionConfig {
  TextKey: string
  TextZh: string
  TextEn: string   // optional / reserved
  Comment: string
}
```
...
**Disk name:** Excel `音频_BGM配置表_Audio_BgmConfig.xlsx`; CSV `Audio_BgmConfig.csv`. Fields: `BgmId` PK; `Context`=`Title`\|`Dig`\|`Combat`; `ClipId` catalog key (filename without ext under `Art/Audio/BGM/`); `Loop` default 1; `Weight` ≥0 (0 excluded); `Volume` 0～1 default 1.

Runtime: `BgmService` + `BgmClipCatalog`; Title on SaveSelect only; Dig on Dig Enter; Combat shared by Defend/PushMap Combat; else Stop.
...
- **背景层（UI-026）：** 子节点顺序 `Background` → `ShopBackdrop` → `ShopBox`。`Background`：Sprite `Art/UI/Meta/Title/Title_Shop_1.png`，`AspectRatioFitter` EnvelopeParent（`aspectRatio` 取 Sprite 宽高比，缺省 16:9）锁定长宽比铺满；`ShopBackdrop` 半透明 Dim（约 `Color(0,0,0,0.55)`，挡点击）；`ShopBox` Image 全透明（仅 UI 父容器）。`ShopAssetBuilder.EnsureShopBackground` 绑定 Sprite。
...
**地图自动接边（MapAutoTile，编辑器刷图约定；非玩法规则；不新增 SPEC_03 D-xxx）：** Tile Palette 刷一片区域时，边界自动换成配套边/角砖。技术：工程已含 `com.unity.2d.tilemap.extras`；权威刷笔为 **Isometric Rule Tile**（Grid=`Isometric` + RotX≈90°；**禁止**把正交 Rule Tile / Terrain Tile 当作本工程等距地图的权威刷笔）。正式资产：`Assets/Art/Maps/RuleTiles/`（Rule Tile SO）；Sprite / 普通 Tile 仍在 `Assets/Art/Maps/Tiles/`；挂到权威刷图 Palette **`Art/Maps/Palettes/FantasyTileset_A`**。一条 Rule Tile = 一套邻接规则（邻格「同类 / 非同类」→ 中心 / 边 / 角输出）。适用范围：**地面及其它装饰/地形配套砖**（道路、悬崖、墙基等按套配置）；**不含** FlowingWater 的 `Water`/`Foam` 层（泡沫接边若需要须另开约定）。**不参与** NavMesh / 空气墙 / `WalkSurface` / `DigMapBounds` / EngageZone / 占领。运行时 Tilemap 仅存解析后的单元格；玩法不读 Rule。**禁止**运行时/已提交 Prefab 引用 `SmallScaleInt/`。作者图（含 `PushMap_Demo_*`）须用 **FantasyTileset_A** / Art Tile 刷图；若 Prefab 仍残留厂商 FantasyTileset（SSI）GUID，用菜单 `Gravedigger2026/Maps/Remap PushMap_Demo_03 SSI Tiles To Art (FantasyTileset_A names)`（`-executeMethod Gravedigger2026.Editor.Maps.MapTileSsiToArtRemapper.RemapPushMapDemo03`）按 **Tile 资源名** 重绑到 Art `Environment/Tiles`、`Animated tiles`、`RuleTiles` 同名砖。**Palette 格子校正：** 菜单 `Gravedigger2026/Maps/Correct FantasyTileset_A From FantasyTileset`（`-executeMethod Gravedigger2026.Editor.Maps.FantasyTilesetALayoutAligner.CorrectFantasyTilesetAFromFantasyTileset`）按厂商盘 `SmallScaleInt/.../Environment/FantasyTileset` 的 **Tile 资源名** 对齐 `FantasyTileset_A` 坐标（只用 Art `Environment/Tiles` 同名砖，不复制 SSI TileSpriteArray 偏移）；特例 **`Ground F4_W`** 原地保留（SSI 无此砖则不删除）；A 独有砖溢到参考包围盒右侧；`RT_WallA` 重钉固定槽。跑完须重开 Tile Palette。刷法见 `Assets/Art/Maps/README.md`。issues：`.scratch/map-auto-edge-tiles/`。

**地图边缘迷雾（MapEdgeFog，地图表现约定；方案 A 已锁；非玩法规则；不新增 SPEC_03 D-xxx）：** 用途是**遮挡可玩区外空白**并兼顾氛围；**不以**屏幕全屏滤镜为主手段（那是 `CameraFogOverlay` / `CameraFogService` 的职责，可叠加、职责分离）。权威做法：**世界空间边缘雾**挂在各地图 Prefab（`Prefabs/Maps/{MapId}`）上——相对 `DigMapBounds` / IsoDiamond 可玩足迹，在外侧放置 **1 个环状软边雾片或至多 4 条边雾片**（`SpriteRenderer` 或带 alpha 渐变的 Quad）。Sorting：**高于**地面 / Water / Foam Tilemap，**低于**单位与战斗特效；材质半透明、同图尽量共用材质实例。配置（灵活、非表型）：Prefab 上 `SerializeField`（贴图、颜色、Alpha、世界尺寸/偏移、启用开关），或共享 SO → `Assets/Settings/Maps/`；贴图源可复用 / 扩展 `Art/Maps/Fogs/Fog_*.png`。性能底线：**静态、零 `Update`**；动画若需要则进材质 `_Time`（极慢 UV/呼吸），禁止每帧 C# 改 Transform/Color；禁止用粒子堆叠当不透明遮罩、禁止每帧重建 RT/遮罩、禁止 URP Volume Fog。**不参与** NavMesh / 空气墙 / `WalkSurface` / EngageZone / 占领。随地图 Instantiate/Destroy，禁止挂 `transform.root` 造成阶段残留。摆放见 `Assets/Art/Maps/README.md`。
...
**MapAutoTile (editor paint convention only; not a gameplay rule; no new SPEC_03 D-xxx):** when painting a region in the Tile Palette, edges/corners auto-swap to matching companion tiles. Tech: project already has `com.unity.2d.tilemap.extras`; canonical brush is **Isometric Rule Tile** (Grid=`Isometric` + RotX≈90°; **do not** treat orthogonal Rule Tile / Terrain Tile as the authority brush for this project's isometric maps). Canonical assets: `Assets/Art/Maps/RuleTiles/` (Rule Tile SOs); sprites / plain Tiles stay under `Assets/Art/Maps/Tiles/`; expose on paint palette **`Art/Maps/Palettes/FantasyTileset_A`**. One Rule Tile = one neighbor rule set (same / not-same → center / edge / corner). Scope: **ground and other décor/terrain companion sets** (roads, cliffs, wall bases, etc. per set); **excludes** FlowingWater `Water`/`Foam` layers (foam auto-edge would need a separate convention). **Does not** participate in NavMesh / AirWall / `WalkSurface` / `DigMapBounds` / EngageZone / capture. Runtime Tilemap stores resolved cells only; gameplay does not read Rule logic. **No** runtime / committed-Prefab refs to `SmallScaleInt/`. Author maps (incl. `PushMap_Demo_*`) must paint with **FantasyTileset_A** / Art Tiles; leftover vendor FantasyTileset (SSI) GUIDs → menu `Gravedigger2026/Maps/Remap PushMap_Demo_03 SSI Tiles To Art (FantasyTileset_A names)` (`-executeMethod Gravedigger2026.Editor.Maps.MapTileSsiToArtRemapper.RemapPushMapDemo03`) remaps by **Tile asset name** to Art `Environment/Tiles`, `Animated tiles`, `RuleTiles`. **Palette layout corrector:** menu `Gravedigger2026/Maps/Correct FantasyTileset_A From FantasyTileset` (`-executeMethod Gravedigger2026.Editor.Maps.FantasyTilesetALayoutAligner.CorrectFantasyTilesetAFromFantasyTileset`) aligns `FantasyTileset_A` cells to vendor `SmallScaleInt/.../Environment/FantasyTileset` by **Tile asset name** (Art `Environment/Tiles` same-name only; does not copy SSI TileSpriteArray offsets); exception **`Ground F4_W`** stays in place (SSI has no such tile — do not remove); A-only tiles overflow to the right of the reference bounds; `RT_WallA` re-pinned to its fixed slot. Re-open Tile Palette after running. Paint guide: `Assets/Art/Maps/README.md`. Issues: `.scratch/map-auto-edge-tiles/`.

**MapEdgeFog (map presentation only; Approach A locked; not a gameplay rule; no new SPEC_03 D-xxx):** purpose is to **cover blank space outside the playable footprint** and add atmosphere; **not** primarily a full-screen filter (that remains `CameraFogOverlay` / `CameraFogService` — may stack; separate duties). Canonical approach: **world-space edge fog** on each map Prefab (`Prefabs/Maps/{MapId}`) — relative to `DigMapBounds` / IsoDiamond playable footprint, place **one soft-edge fog ring or up to four edge strips** (`SpriteRenderer` or alpha-gradient Quad) outside the diamond. Sorting: **above** ground / Water / Foam Tilemaps, **below** units and combat VFX; translucent materials; share material instances per map when possible. Config (flexible, non-table): Prefab `SerializeField` (sprite, color, alpha, world size/offset, enable) and/or shared SO under `Assets/Settings/Maps/`; art may reuse / extend `Art/Maps/Fogs/Fog_*.png`. Perf floor: **static, zero `Update`**; optional motion via material `_Time` only (slow UV/pulse) — no per-frame C# Transform/Color; no particle stacks as opaque cover; no per-frame RT/mask rebuild; no URP Volume Fog. **Does not** participate in NavMesh / AirWall / `WalkSurface` / EngageZone / capture. Lifetime follows map Instantiate/Destroy; do **not** parent under `transform.root` (stage leaks). Placement guide: `Assets/Art/Maps/README.md`.
...
**2D Zombie Pack clip 行绑定（v0.83.97 方案 A）：** `MonsterModel_02/04/05/07/08` spritesheet 自上而下为 E/SE/S/SW/W/NW/N/NE。`Play("{基名}_{dir}")` 的 `.anim` 必须引用该视觉行的 Sprite；与上表 `DirIndex` **不是**同一套行号。Creator 烘焙曾把 `Attack*_SE` 绑到自上而下第 7 行（N），表现为朝上/左上挥砍。
...
| Fake death | PushMap / SearchExtract `MonsterCombatDead` uses same arc (smash per gate); after flight + latch still Delay→reverse revive (D-074 unchanged except SearchExtract point-clear) |
...
**2D Zombie Pack clip row bind (v0.83.97 Approach A):** `MonsterModel_02/04/05/07/08` sheets are top-to-bottom E/SE/S/SW/W/NW/N/NE. `.anim` files for `Play("{base}_{dir}")` must reference that visual row; this is **not** the `DirIndex` row numbering above. Creator bake had bound `Attack*_SE` to the 7th row from the top (N), so SE attacks faced up / up-left.
