...
**进档难度 Hub（UI-029 / D-081 方案 C）：** **权威源**独立 Prefab `Assets/Prefabs/Meta/InSaveShellPanel.prefab`；`MetaShellRoot` / Boot 仅嵌套其 **Prefab Instance**（禁止把进档壳整树 bake 进 `MetaShellRoot`）。菜单 `Ensure InSaveShellPanel`：补缺结构、把根上嵌套改为 Instance、**不得**用 `MetaShellRoot` 反向覆盖权威 Prefab、**不得**在已有 `ColumnsScroll`+`DescriptionText` 时拆毁手调难度栏。进档默认中心面 `DifficultySelectHost`：`ColumnsScroll` 下等宽三栏普通/困难/地狱**同屏**（各约 1/3 视口）；悬停 Column 显示难度描述（Demo 写死）；**仅普通**点击 → `TryEnterLevel`（默认末项去重 LevelId）并打开 UI-031；困难/地狱可悬停、点击 Toast「还未制作」。`NormalColumn` 难度贴图 `Image.preserveAspect=true`。**不再**把 `LevelSelectPanel` 嵌于栏内作为主路径（Prefab 可保留、默认隐藏）。**无**中央 `MapHost` / 地图底图。关卡运行中 Hide Hub。工具「关卡」/ 推图结算 Continue 打开同一 Hub。左下商店/装备/魔法书/返回与右上工具锚点不变。issues：`.scratch/insave-difficulty-hub/`。
