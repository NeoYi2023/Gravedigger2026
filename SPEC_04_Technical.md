...
**BGM（方案 A，§9.29）：** `ConfigCsvRepository` 加载 `Audio_BgmConfig.csv`（Mode1/Mode2）；纯 C# `BgmService` + SO `BgmClipCatalog`（引用 `Art/Audio/BGM`，不走 Resources）；Boot/`MetaShellController` 持有 `AudioSource`。播曲规则见 [SPEC_03 §3.4](SPEC_03_GameRules.md)：`ShowSaveSelect`→Title；`EnterShell`/`LevelEnded`/非 Dig·Combat→Stop；`DigStageModule` Enter/Exit→Dig/Stop；`DefendSessionService`/`PushMapSessionService` `PhaseChanged` 仅 `Combat`→Combat 否则 Stop。同 Context 幂等。
