...
**进档难度 Hub（UI-029 / D-081 方案 C）：** **权威源**独立 Prefab `Assets/Prefabs/Meta/InSaveShellPanel.prefab`；`MetaShellRoot` / Boot 仅嵌套其 **Prefab Instance**（禁止把进档壳整树 bake 进 `MetaShellRoot`）。菜单 `Ensure InSaveShellPanel`：补缺结构、把根上嵌套改为 Instance、**不得**用 `MetaShellRoot` 反向覆盖权威 Prefab、**不得**在已有 `ColumnsScroll`+`DescriptionText` 时拆毁手调难度栏。进档默认中心面 `DifficultySelectHost`：`ColumnsScroll` 下等宽三栏普通/困难/地狱**同屏**（各约 1/3 视口）；悬停 Column 显示难度描述（Demo 写死）；**仅普通**点击 → `TryEnterLevel`（默认末项去重 LevelId）并打开 UI-031；困难/地狱可悬停、点击 Toast「还未制作」。三栏均使用作者指定难度贴图；其中 `NormalColumn` / `HardColumn` / `HellColumn` 均保留各自 `Image.m_Sprite`，并统一 `Image.preserveAspect=true`，禁止回退为纯色无图背景。**不再**把 `LevelSelectPanel` 嵌于栏内作为主路径（Prefab 可保留、默认隐藏）。**无**中央 `MapHost` / 地图底图。关卡运行中 Hide Hub。工具「关卡」/ 推图结算 Continue 打开同一 Hub。左下商店/装备/魔法书/返回与右上工具锚点不变。issues：`.scratch/insave-difficulty-hub/`。
...
- DigStageSummary（UI-011）：每条奖励为 `DigSummaryItemCell`（方图标 + 右下数量 + 下方名称）；躯体名=`DisplayName`（空则 `BodyPartId`），图标=`DigBodyArtLoader`；精魂/非躯体名=`Id`，图标=`ItemIconLoader.LoadFromCatalog`；`Body` 网格每行最多 **5**。
- 具体数值行 **TBD**。
...
| `AutoMfgPileFloorYPx` | 自动制造尸骸堆地板 Y | `-300` | 2D 演出层堆地板相对屏幕中心 Y（1920×1080） |
| `AutoMfgSoldierLandYPx` | 自动制造士兵落地 Y | `-410` | 复活士兵落地相对屏幕中心 Y（法阵下方） |
| `AutoMfgBodyMaxEdgePx` | 自动制造躯体适配边长 | `49` | StepA 躯体 Image 最长边像素（约原 140 的 35%） |
| `AutoMfgSoldierMaxEdgePx` | 自动制造士兵固定边长 | `64` | StepB 问号/士兵 Image sizeDelta 宽高（再 × VisualScale） |
...
| `AutoMfgSoldierShadowOffsetYPx` | 自动制造士兵影子相对 Y | `-40` | 影子相对士兵中心的本地 Y |
...
| `AutoMfgPileFloorYPx` | 自动制造尸骸堆地板 Y | `-300` | 2D pile floor Y vs screen center (1920×1080) |
| `AutoMfgSoldierLandYPx` | 自动制造士兵落地 Y | `-410` | Revived soldier land Y vs screen center (below circle) |
| `AutoMfgBodyMaxEdgePx` | 自动制造躯体适配边长 | `49` | StepA body Image max edge px (~35% of prior 140) |
| `AutoMfgSoldierMaxEdgePx` | 自动制造士兵固定边长 | `64` | StepB mystery/soldier Image sizeDelta W/H (then × VisualScale) |
...
| `AutoMfgSoldierShadowOffsetYPx` | 自动制造士兵影子相对 Y | `-40` | Shadow local Y vs soldier center |
...
**开战镜头（v0.83.93 / HoldFraming v0.84.27 / 样例锁定 v0.84.28）：** 复用 `PushMapCameraFollowController`。**进圈前 / Continue 后接近** = 跟随地图 `CameraFollowPath` 与忠诚士兵投影（同 PushMap Combat）。**点激活～GatherCountdown** = **HoldFraming**（`SearchExtractHoldFramingSolver` 视口包围+迟滞；常量 `SearchExtractHold*` ← §9.20b，SE-CAM-03 锁定初值）；UI-032 冻结；Continue 回轨。PushMap **不**调用 Hold API。
