...
{
    /// <summary>
    /// MP-04/05: loyal advance via FlowField; engage â†?GoalKind=AttackSlot (SPEC_03 Â§3.12/Â§3.14).
    /// Samples MassMoveScheduler steer; applies NavMeshAgent.Move â€?no per-frame SetDestination.
    /// Capture-zone monsters do NOT pause advance. Rebels do not advance.
    /// PM-12 (Approach B, SPEC_04 Â§9.22): WarriorCombat scheme D â€?melee windup â†?
    /// TryConfirmMeleeHit; ranged â†?shared ProjectileView soft-hit â†?TryConfirmRangedHit
    ...
    /// D-069: Skill_03 burst occupies this attack channel (3Ã— scheme D) when CD ready.
    /// D-073 SE-09: new-target acquire may Warp behind farthest enemy (rules pick; View Warp).
    /// PM-13: CombatDead â†?PlayDie + stop acting (aligned with Defend WarriorAgentView).
    /// PM-13: CombatDead â†?PlayDie + stop acting (aligned with Defend WarriorAgentView).
    ...
    public sealed class PushMapAdvanceView : MonoBehaviour
    {
        ...
        /// <summary>
        /// SPEC_03 Â§3.14 v0.74.10: a rival must be closer than the claimed target by more
        /// than this margin to steal the claim â€?dense packs + soft-collision jostle
        ...
        /// <summary>Last MassMove steer XZ (LateUpdate); drives IsRun â€?not NavMeshAgent.velocity (SPEC_04 Â§15.5).</summary>
        private Vector3 _lastSteerDirXZ;
        /// <summary>Last MassMove pre-detour desired; drives DirIndex while moving (SPEC_04 Â§15.5 v0.83.31).</summary>
            ...
            float chaseMoveSpeedMult = ClassConfigRow.DefaultChaseMoveSpeedMult)
        {
            ...
            // Facing via Animator DirIndex in PushMap as in Defend (SPEC_04 Â§15.2).
        ...
        /// <summary>
        /// Effective speed: base Ã— ChaseMoveSpeedMult only when GoalKind=AttackSlot (SPEC_03 Â§3.12).
        /// </summary>
        ...
        /// v0.74.10 sticky hysteresis (SPEC_03 Â§3.14): while the claimed
        ...
        public bool TryGetEngageMonster(out PushMapMonsterAgentView monster)
        {
            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                // Detect = max(weapon reach, monster AlertRadius) (SPEC_03 Â§3.12 v0.82.55 C).
        ...
        /// Avoids Releaseâ†’full-ring fail â†?FormationHome thrash.
        /// </summary>
        ...
        private void Update()
        {
            ...
            // PM-13: CombatDead / PermanentDeath mark â†?PlayDie once and stop acting.
            if (!IsCombatActive)
            {
        ...
        /// <summary>
        /// PM-12 scheme D: engaged (AttackSlot claim on a living monster) + in AttackRange â†?
        ...
        private bool TryResolveCombatTarget(out PushMapMonsterAgentView target)
        {
            ...
            // (SPEC_03 Â§3.12 v0.82.57). Still allow the swing if the detect target is in reach.
            if (_scheduler == null ||
                !_scheduler.TryGetGoal(_moveId, out var kind, out _) ||
                kind != GoalKind.AttackSlot ||
        ...
        /// once at PlayAttack (SPEC_04 Â§15.5 v0.83.31).
        /// </summary>
        private void TickAnimPresentation()
        {
            ...
            var inWindup = _attackPhase == AttackPhase.Windup ||
                           _attackPhase == AttackPhase.BurstRecover;
            // MassMove uses Move()+ResetPath â€?velocityâ‰?; use steer like monsters (SPEC_04 Â§15.5).
        ...
        /// <summary>SPEC_04 Â§15.5: distance for attackâ†’run interrupt gate (Objective / missing â†?+âˆ?.</summary>
        ...
        private void LateUpdate()
        {
            ...
            // No SetDestination â€?follow scheduler steer (Objective field or AttackSlot).
            if (_agent.hasPath)
            {
                _agent.ResetPath();
            }
        ...
        /// <summary>
        /// SPEC_03 Â§3.12 v0.84.25/26: arm exclude-id when AttackSlot chase is out of range and stuck.
        /// </summary>
