...
namespace Gravedigger2026.Core.Pathing
{
    ...
    public sealed class SoftCollisionService
    {
        ...
        /// <summary>
        /// SC-04 over-budget fallback knob (SPEC_04 §9.7 fallback ④): multiplier on the
        /// neighbor query radius (default 1.0 = <see cref="SpatialHash2D.RecommendedQueryRadius"/>).
        /// Lower (e.g. 0.75) shrinks the repulsion neighborhood to cut query cost; ≤0 disables
        /// neighbor gathering entirely. Clamped ≥0 at use; repulsion strength is unaffected.
        /// </summary>
        public float QueryRadiusScale { get; set; } = 1f;
        ...
        public void Tick(float dt, int maxBodiesPerFrame = DefaultMaxBodiesPerFrame)
        {
            ...
            var radiusScale = Mathf.Max(0f, QueryRadiusScale);
