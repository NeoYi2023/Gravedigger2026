...
namespace Gravedigger2026.Core.Pathing
{
    ...
    public sealed class FlowFieldService
    {
        ...
        /// <summary>
        /// Direction field (SPEC_04 §9.7 v0.74.11, approach A): central-difference gradient
        /// of the integration field, stored as the normalized negative gradient. Blocked /
        /// out-of-range / unreachable neighbors substitute the cell's own cost, degrading to
        /// a one-sided difference. Continuous directions replace the old cheapest-neighbor
        /// 8-dir quantization (45° steps doglegged on non-45° bearings and flip-flopped
        /// cell-to-cell along jagged rotated walls into M-shaped tracks).
        /// </summary>
        private void BuildDirections()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var self = _integration[i];
                var gx = IntegrationForGradient(cx + 1, cz, self) - IntegrationForGradient(cx - 1, cz, self);
                var gz = IntegrationForGradient(cx, cz + 1, self) - IntegrationForGradient(cx, cz - 1, self);
                var dir = new Vector2(-gx, -gz);
        ...
        private float IntegrationForGradient(int cellX, int cellZ, float fallback)
        {
            var index = IndexOf(cellX, cellZ);
            if (index < 0 || !_walkable[index] || float.IsInfinity(_integration[index]))
            {
                return fallback;
            }
