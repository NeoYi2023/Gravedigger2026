// Facing-stabilizer correctness checks (v0.75.10, SPEC_04 §15.5).
...
namespace Gravedigger2026.Gameplay.PushMap
{
    ...
    {
        private const float Hysteresis = 12f; // FacingHysteresisDegrees
        ...
        public static string RunAll()
        {
            ...
            for (var i = 0; i < 50; i++)
            {
                ...
                Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(0, Dir(deg), Hysteresis) == 0,
                    $"boundary oscillation holds E (deg={deg})");
            }

            // 2) Beyond hysteresis switches: E → raw 126° (delta 36 > 34.5) → SE(6).
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(0, Dir(126f), Hysteresis) == 6,
                "beyond hysteresis switches E→SE");

            // 3) Tight threshold (22.5+12=34.5): current N (center 0°); 34.4° keeps, 34.6° switches.
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(3, Dir(34.4f), Hysteresis) == 3,
                "34.4° within hysteresis keeps N");
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(3, Dir(34.6f), Hysteresis) == 4,
                "34.6° past hysteresis switches N→NE");

            // 4) Zero vector keeps current (DirIndexFromXZ default S must not hijack).
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(0, Vector3.zero, Hysteresis) == 0,
                "zero vector keeps E");
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(5, Vector3.zero, Hysteresis) == 5,
                "zero vector keeps NW");

            // 5) Invalid current → candidate passthrough (first-frame init).
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(-1, Dir(270f), Hysteresis) == 1,
                "invalid current -1 → W");
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(8, Dir(180f), Hysteresis) == 2,
                "invalid current 8 → S");

            // 6) Round-trip: DirIndexToUnitXZ feeds DirIndexFromXZ back to the same index.
            for (var i = 0; i < 8; i++)
            {
                var back = Defend.WarriorAnimView.DirIndexFromXZ(
                    PushMapMonsterFacingStabilizer.DirIndexToUnitXZ(i));
                Check(sb, back == i, $"DirIndexToUnitXZ round-trip {i} (got {back})");
            }

            // 7) Full 180° flip switches immediately: E → W.
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(0, Dir(270f), Hysteresis) == 1,
                "full flip E→W");

            // 8) Legitimate large turn not delayed: N → raw 50° (delta 50 > 34.5) → NE(4).
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(3, Dir(50f), Hysteresis) == 4,
                "large turn N→NE switches");

            // 9) Zero hysteresis degrades to plain quantization: N, raw 23° (delta 23 > 22.5) → NE.
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(3, Dir(23f), 0f) == 4,
                "zero hysteresis = plain quantization");
            //    ...and with the locked 12° the same 30° steer holds N.
            Check(sb, PushMapMonsterFacingStabilizer.StabilizeDirIndex(3, Dir(30f), Hysteresis) == 3,
