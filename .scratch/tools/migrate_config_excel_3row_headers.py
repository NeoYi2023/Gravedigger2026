...
FIELDS: dict[str, dict[str, tuple[str, str]]] = {
    ...
    "Defend_MonsterConfig": {
        ...
        "MonsterType": ("怪物类型", "1=普通/2=精英/3=BOSS；空→1；非法失败；异于 PushMap IsBoss"),
        "AggroMode": ("仇恨模式", "ActiveChase|PassiveChase|StationaryActive|StationaryPassive；空→ActiveChase"),
        "AlertRadius": ("警戒半径", "≥0；空→AttackRange"),
        "BodyRadius": ("占地半径", "≥0；空→0.35；PushMap 刷出散开与 NavMeshAgent 避障半径"),
        "PushCoefficient": ("推开系数", "≥0；缺省1；SoftCollision 排斥冲量缩放（邻域贡献×对方系数）；不改占地圆"),
        "RepulsionScale": ("排斥强度系数", "≥0；缺省1；SoftCollision 单体排斥（有效=全局×本系数）"),
        "FacingYawFlip": ("朝向整圈翻转", "0不转;1=(DirIndex+4)%8;缺省0"),
        "MaxHP": ("怪物血量", "生成时初始化 maxHP / 当前 HP"),
        "MoveSpeed": ("怪物走速度", "世界单位/秒；走态移速"),
        "RunSpeed": ("怪物跑速度", "世界单位/秒；缺/≤0→回退 MoveSpeed"),
        "WalkToRunSeconds": ("走转跑时间", "秒；持续移动达到后切跑；缺→0.5；0=一开跑"),
        "ActiveMoveMult": ("主动追击移速倍率", "ActiveChase 时×当前 gait 移速；缺→1"),
        "PassiveMoveMult": ("被动追击移速倍率", "PassiveChase 时×当前 gait 移速；缺→1"),
        ...
        "Skills": ("怪物技能", "SkillId;CdSeconds|…；禁止用 _ 后缀表示 CD"),
        "NormalAttackAnims": ("普通攻击动作调用", "基名|基名|…；每次 PlayAttack 随机；空→Attack1"),
        "WalkAnims": ("走", "BlendTree 状态名|…；Bind/复活完成各抽一次；空→WalkBT"),
        "RunAnims": ("跑", "BlendTree 状态名|…；与走同生命周期；跑态移动时播"),
        "LootDrop": ("怪物掉落", "击杀产出；编码 Id;Count|Id;Count|…"),
    },
    "PushMap_PushMapGameplayConfig": {
        ...
        "CaptureLoot": ("占领默认掉落", "可选；编码 Id;Count|…"),
