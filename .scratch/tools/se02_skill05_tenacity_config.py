# -*- coding: utf-8 -*-
"""SE-02: SkillEffect_05 rows + Skill_05 EffectImplemented=1; bake Mode2 CSV."""
...
SKILL05_EFFECTS = {
    "SkillEffect_05_1": ("CheatDeathInvincible", "InvincibleSeconds=1", "OnWarriorWouldDie"),
    "SkillEffect_05_2": ("CheatDeathInvincible", "InvincibleSeconds=2", "OnWarriorWouldDie"),
    "SkillEffect_05_3": ("CheatDeathInvincible", "InvincibleSeconds=3", "OnWarriorWouldDie"),
    "SkillEffect_05_4": ("CheatDeathInvincible", "InvincibleSeconds=4", "OnWarriorWouldDie"),
    "SkillEffect_05_5": ("CheatDeathInvincible", "InvincibleSeconds=5", "OnWarriorWouldDie"),
}
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        if sid is None:
            continue
        sid = str(sid).strip()
        if sid not in SKILL05_EFFECTS:
            continue
        kind, params, hook = SKILL05_EFFECTS[sid]
        ws.cell(r, 3, kind)
        ws.cell(r, 4, params)
        ws.cell(r, 5, hook)
        updated += 1
    ...
    wb.save(EFFECT_XLSX)
    print(f"Saved {EFFECT_XLSX.name}: updated {updated} SkillEffect_05 rows")
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_05":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_05 EffectImplemented=1 for {updated} rows")
