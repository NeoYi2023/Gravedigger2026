#!/usr/bin/env python3
"""Inject FormationClassZones into PushMap_Demo_* (SPEC v0.84.05 Approach B).

Anchor: WP_Start local ≈ (-0.13, 0.05, 0.11) under map root (parents at origin).
ClassId list: Mode2 Manufacture_ClassConfig.csv.
"""
...
HALF = (3.85, 2.0)
# Match DefendAssetBuilder.NewClassZoneRelXZ (+ fallback grid for Base*).
KNOWN_REL = {
...
# Stable high IDs under Int64.MaxValue (9223372036854775807). SearchExtract uses 92006…;
# use 88006… for PushMap inject to avoid collision.
...
def patch_prefab(path: Path, class_ids: list[str]) -> None:
    ...
    if "m_Name: FormationClassZones" in text:
        print(f"SKIP (already has zones): {path.name}")
        return

    block, zones_tf = build_block(class_ids)

    # Insert child under map root Transform children list.
    pattern = re.compile(
    ...
    print(f"OK injected {len(class_ids)} zones → {path.name}")
