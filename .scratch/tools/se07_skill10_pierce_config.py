# -*- coding: utf-8 -*-
"""SE-07: add SkillEffect_10_1..5 + Skill_10 EffectImplemented=1; bake Mode2 CSV."""
...
SKILL10_EFFECTS = {
    "SkillEffect_10_1": (
        ...
        "ExtraHitCount=1|DamageMul=1",
        "OnProjectileHit",
    ),
    "SkillEffect_10_2": (
        "贯穿 Lv2：再穿 2 名；100%",
        "RangedPierceExtraHits",
        "ExtraHitCount=2|DamageMul=1",
        "OnProjectileHit",
    ),
    "SkillEffect_10_3": (
        "贯穿 Lv3：再穿 3 名；100%",
        "RangedPierceExtraHits",
        "ExtraHitCount=3|DamageMul=1",
        "OnProjectileHit",
    ),
    "SkillEffect_10_4": (
        "贯穿 Lv4：再穿 4 名；100%",
        "RangedPierceExtraHits",
        "ExtraHitCount=4|DamageMul=1",
        "OnProjectileHit",
    ),
    "SkillEffect_10_5": (
        "贯穿 Lv5：再穿 5 名；100%",
        "RangedPierceExtraHits",
        "ExtraHitCount=5|DamageMul=1",
        "OnProjectileHit",
    ),
}
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        sid = str(sid).strip()
        existing[sid] = r
        if sid.startswith("SkillEffect_09_") or sid.startswith("SkillEffect_08_"):
            insert_after = max(insert_after, r)

    updated = 0
    added = 0
    for sid, (notes, kind, params, hook) in SKILL10_EFFECTS.items():
        if sid in existing:
            r = existing[sid]
            ws.cell(r, 2, notes)
            ws.cell(r, 3, kind)
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_10":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_10 EffectImplemented=1 for {updated} rows")
