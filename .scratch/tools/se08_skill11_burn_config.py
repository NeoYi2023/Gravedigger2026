# -*- coding: utf-8 -*-
"""SE-08: add SkillEffect_11_1..5 + Skill_11 EffectImplemented=1; bake Mode2 CSV."""
...
BURN_PARAMS = "TickDamageMul=0.2|TickIntervalSeconds=1|DurationSeconds={dur}|StackMode=RefreshDuration"

SKILL11_EFFECTS = {
    "SkillEffect_11_1": (
        "灼烧 Lv1：20% NAP/1s × 2s；叠时",
        "OnAaHitApplyBurn",
        BURN_PARAMS.format(dur=2),
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_11_2": (
        "灼烧 Lv2：持续 3s",
        "OnAaHitApplyBurn",
        BURN_PARAMS.format(dur=3),
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_11_3": (
        "灼烧 Lv3：持续 4s",
        "OnAaHitApplyBurn",
        BURN_PARAMS.format(dur=4),
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_11_4": (
        "灼烧 Lv4：持续 5s",
        "OnAaHitApplyBurn",
        BURN_PARAMS.format(dur=5),
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_11_5": (
        "灼烧 Lv5：持续 6s",
        "OnAaHitApplyBurn",
        BURN_PARAMS.format(dur=6),
        "OnWarriorAaHitConfirm",
    ),
}
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        sid = str(sid).strip()
        existing[sid] = r
        if sid.startswith("SkillEffect_10_") or sid.startswith("SkillEffect_09_"):
            insert_after = max(insert_after, r)

    updated = 0
    added = 0
    for sid, (notes, kind, params, hook) in SKILL11_EFFECTS.items():
        if sid in existing:
            r = existing[sid]
            ws.cell(r, 2, notes)
            ws.cell(r, 3, kind)
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_11":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_11 EffectImplemented=1 for {updated} rows")
