# -*- coding: utf-8 -*-
"""SE-06: add SkillEffect_09_1..5 + Skill_09 EffectImplemented=1; bake Mode2 CSV."""
...
SKILL09_EFFECTS = {
    "SkillEffect_09_1": (
        "渐入佳境 Lv1：+3%/10s，cap 60%",
        "StackingOutgoingMulTimed",
        "StackBonus=0.03|MaxTotalBonus=0.6|TickSeconds=10",
        "OnSkillInternalCooldown",
    ),
    "SkillEffect_09_2": (
        "渐入佳境 Lv2：+5%/10s，cap 60%",
        "StackingOutgoingMulTimed",
        "StackBonus=0.05|MaxTotalBonus=0.6|TickSeconds=10",
        "OnSkillInternalCooldown",
    ),
    "SkillEffect_09_3": (
        "渐入佳境 Lv3：+7%/10s，cap 60%",
        "StackingOutgoingMulTimed",
        "StackBonus=0.07|MaxTotalBonus=0.6|TickSeconds=10",
        "OnSkillInternalCooldown",
    ),
    "SkillEffect_09_4": (
        "渐入佳境 Lv4：+9%/10s，cap 60%",
        "StackingOutgoingMulTimed",
        "StackBonus=0.09|MaxTotalBonus=0.6|TickSeconds=10",
        "OnSkillInternalCooldown",
    ),
    "SkillEffect_09_5": (
        "渐入佳境 Lv5：+12%/10s，cap 60%",
        "StackingOutgoingMulTimed",
        "StackBonus=0.12|MaxTotalBonus=0.6|TickSeconds=10",
        "OnSkillInternalCooldown",
    ),
}
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        sid = str(sid).strip()
        existing[sid] = r
        if sid.startswith("SkillEffect_08_"):
            insert_after = max(insert_after, r)

    updated = 0
    added = 0
    for sid, (notes, kind, params, hook) in SKILL09_EFFECTS.items():
        if sid in existing:
            r = existing[sid]
            ws.cell(r, 2, notes)
            ws.cell(r, 3, kind)
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_09":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_09 EffectImplemented=1 for {updated} rows")
