# -*- coding: utf-8 -*-
"""SE-05: add SkillEffect_08_1..5 + Skill_08 EffectImplemented=1; bake Mode2 CSV."""
...
SKILL08_EFFECTS = {
    "SkillEffect_08_1": (
        "精英克制 Lv1：EnemySingle Elite +50%",
        "OutgoingMulVsMonsterType",
        "MonsterType=Elite|Mul=1.5",
        "OnOutgoingDamageSettle",
    ),
    "SkillEffect_08_2": (
        "精英克制 Lv2：Elite +60%",
        "OutgoingMulVsMonsterType",
        "MonsterType=Elite|Mul=1.6",
        "OnOutgoingDamageSettle",
    ),
    "SkillEffect_08_3": (
        "精英克制 Lv3：Elite +70%",
        "OutgoingMulVsMonsterType",
        "MonsterType=Elite|Mul=1.7",
        "OnOutgoingDamageSettle",
    ),
    "SkillEffect_08_4": (
        "精英克制 Lv4：Elite +80%",
        "OutgoingMulVsMonsterType",
        "MonsterType=Elite|Mul=1.8",
        "OnOutgoingDamageSettle",
    ),
    "SkillEffect_08_5": (
        "精英克制 Lv5：Elite +90%",
        "OutgoingMulVsMonsterType",
        "MonsterType=Elite|Mul=1.9",
        "OnOutgoingDamageSettle",
    ),
}
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        sid = str(sid).strip()
        existing[sid] = r
        if sid.startswith("SkillEffect_07_"):
            insert_after = max(insert_after, r)

    updated = 0
    added = 0
    for sid, (notes, kind, params, hook) in SKILL08_EFFECTS.items():
        if sid in existing:
            r = existing[sid]
            ws.cell(r, 2, notes)
            ws.cell(r, 3, kind)
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_08":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_08 EffectImplemented=1 for {updated} rows")
