# -*- coding: utf-8 -*-
"""SE-09: add SkillEffect_12_1..5 + Skill_12 EffectImplemented=1; bake Mode2 CSV."""
...
SKILL12_EFFECTS = {
    "SkillEffect_12_1": (
        "瞬移 Lv1：最远敌背后；BaseCD=60",
        "RetargetFarthestTeleportBehind",
        "",
        "OnWarriorTargetAcquired",
    ),
    "SkillEffect_12_2": (
        "瞬移 Lv2：BaseCD=50",
        "RetargetFarthestTeleportBehind",
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        sid = str(sid).strip()
        existing[sid] = r
        if sid.startswith("SkillEffect_11_") or sid.startswith("SkillEffect_10_"):
            insert_after = max(insert_after, r)

    updated = 0
    added = 0
    for sid, (notes, kind, params, hook) in SKILL12_EFFECTS.items():
        if sid in existing:
            r = existing[sid]
            ws.cell(r, 2, notes)
            ws.cell(r, 3, kind)
            ...
            ws.cell(r, 5, hook)
            updated += 1
            print(f"PATCH {sid} -> {kind} hook={hook}")
        else:
            insert_after += 1
            ...
            added += 1
            print(f"ADD {sid} -> {kind} hook={hook}")
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_12":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_12 EffectImplemented=1 for {updated} rows")
