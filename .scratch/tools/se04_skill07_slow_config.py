# -*- coding: utf-8 -*-
"""SE-04: add SkillEffect_07_1..5 + Skill_07 EffectImplemented=1; bake Mode2 CSV."""
...
SKILL07_EFFECTS = {
    "SkillEffect_07_1": (
        "冰冻 Lv1：AOE 减速 2s；内置 CD 10s",
        "OnAaHitAoeSlow",
        "Radius=1.5|SlowMoveMul=0.5|SlowAttackMul=0.5|DurationSeconds=2|InternalCooldownSeconds=10",
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_07_2": (
        "冰冻 Lv2：减速 3s",
        "OnAaHitAoeSlow",
        "Radius=1.5|SlowMoveMul=0.5|SlowAttackMul=0.5|DurationSeconds=3|InternalCooldownSeconds=10",
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_07_3": (
        "冰冻 Lv3：减速 4s",
        "OnAaHitAoeSlow",
        "Radius=1.5|SlowMoveMul=0.5|SlowAttackMul=0.5|DurationSeconds=4|InternalCooldownSeconds=10",
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_07_4": (
        "冰冻 Lv4：减速 5s",
        "OnAaHitAoeSlow",
        "Radius=1.5|SlowMoveMul=0.5|SlowAttackMul=0.5|DurationSeconds=5|InternalCooldownSeconds=10",
        "OnWarriorAaHitConfirm",
    ),
    "SkillEffect_07_5": (
        "冰冻 Lv5：减速 6s",
        "OnAaHitAoeSlow",
        "Radius=1.5|SlowMoveMul=0.5|SlowAttackMul=0.5|DurationSeconds=6|InternalCooldownSeconds=10",
        "OnWarriorAaHitConfirm",
    ),
}
...
def patch_effect_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        ...
        sid = str(sid).strip()
        existing[sid] = r
        if sid.startswith("SkillEffect_06_"):
            insert_after = max(insert_after, r)

    updated = 0
    added = 0
    for sid, (notes, kind, params, hook) in SKILL07_EFFECTS.items():
        if sid in existing:
            r = existing[sid]
            ws.cell(r, 2, notes)
            ws.cell(r, 3, kind)
...
def patch_skill_config() -> None:
    ...
    for r in range(4, ws.max_row + 1):
        sid = ws.cell(r, 1).value
        if str(sid).strip() != "Skill_07":
            continue
        ws.cell(r, 12, 1)
        updated += 1
    wb.save(SKILL_XLSX)
    print(f"Saved {SKILL_XLSX.name}: Skill_07 EffectImplemented=1 for {updated} rows")
