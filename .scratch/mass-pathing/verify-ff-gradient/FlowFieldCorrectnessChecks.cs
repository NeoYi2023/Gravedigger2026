...
namespace Gravedigger2026.Core.Pathing
{
    ...
    public static class FlowFieldCorrectnessChecks
    {
        ...
        /// <summary>
        /// v0.74.11 gradient direction field (SPEC_04 §9.7): on a bearing that is not a
        /// multiple of 45° (≈26.6°, the sample map's 2:1-diamond corridor slope), advance
        /// must track the true bearing — the old cheapest-neighbor 8-dir field doglegged
        /// (L-shape) and flip-flopped along jagged edges (M-shape).
        /// </summary>
        private static void CheckStraightishPath(StringBuilder sb)
        {
            var field = new FlowFieldService();
            field.Configure(Vector3.zero, new Vector2(5f, 5f), 0.5f);
            var goal = Vector3.zero;
            field.Rebuild(goal, StubFullyWalkableMask.Instance);

            var bearingSamples = new[]
            {
                new Vector3(3.33f, 0f, 1.67f),
                new Vector3(1.02f, 0f, 0.51f),
            };
            const float minBearingDot = 0.978f; // cos(12°)
            foreach (var p in bearingSamples)
            {
                var dir = field.SampleDir(p);
                var toGoal = new Vector2(-p.x, -p.z);
                toGoal.Normalize();
                var dot = dir.x * toGoal.x + dir.y * toGoal.y;
                if (dir.sqrMagnitude < 1e-6f || dot < minBearingDot)
                {
                    sb.AppendLine(
                        $"Bearing misaligned at ({p.x:F2},{p.z:F2}): dir ({dir.x:F2},{dir.y:F2}) " +
                        $"vs true ({toGoal.x:F2},{toGoal.y:F2}).");
                }
            }

            var start = new Vector3(3.33f, 0f, 1.67f);
            var lineX = start.x - goal.x;
            var lineZ = start.z - goal.z;
            var lineLenSq = lineX * lineX + lineZ * lineZ;
            var pos = start;
            var maxDeviation = 0f;
            var reached = false;
            for (var step = 0; step < 80; step++)
            {
                ...
                if (dir.sqrMagnitude < 1e-6f)
                {
                    var gx = pos.x - goal.x;
                    var gz = pos.z - goal.z;
                    reached = gx * gx + gz * gz <= 1.0f;
                    break;
                }

                pos += new Vector3(dir.x, 0f, dir.y) * field.CellSize;

                var relX = pos.x - goal.x;
                var relZ = pos.z - goal.z;
                var t = Mathf.Clamp((relX * lineX + relZ * lineZ) / lineLenSq, 0f, 1f);
                var devX = pos.x - (goal.x + lineX * t);
                var devZ = pos.z - (goal.z + lineZ * t);
                var deviation = Mathf.Sqrt(devX * devX + devZ * devZ);
            ...
            {
                sb.AppendLine(
                    $"Straightish path did not reach goal — ended ({pos.x:F2},{pos.z:F2}).");
            }

            if (maxDeviation > 0.5f)
            {
                sb.AppendLine(
                    $"Dogleg/zigzag regression: max lateral deviation {maxDeviation:F2} > 0.5 " +
                    "(45°-quantized direction field artifact).");
            }
        }
