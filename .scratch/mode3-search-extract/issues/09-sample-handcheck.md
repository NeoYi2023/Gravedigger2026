...
- [ ] 进圈 → 倒计时 + 刷怪 + 重定位
  - **v0.84.00 刷怪时间轴：** 点 1 四行独立配方，约 **5 / 10 / 15 / 20s** 分别从 `SP_01`～`SP_04` 各刷一次（`RepeatSpawnCount=0`，无第二次）；**不是**链式 Delay+Interval
  - 可选回归：临时把某行改为 `Interval=5 Repeat=1`，应在首刷后再隔 5s 同点同怪再刷一次；点胜利后停刷
...
3. 观察顶栏倒计时、方向刷怪、布阵重定位；日志无 PushMap Capture
   - Console 应约在激活后 5/10/15/20s 见四次 `WaveSpawn`（Wave=1..4）；同 Wave 不应再刷（样例 Repeat=0）
