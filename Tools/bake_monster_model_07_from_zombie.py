#!/usr/bin/env python3
"""Bake MonsterModel_07 art from ZombieFemale6 PNGs + MonsterModel_05 clip template.

When Unity Editor is unavailable, this:
1. Copies PNGs from SmallScaleInt ZombieFemale6 (1920x1024, 128px/cell)
...
Run from repo root: python Tools/bake_monster_model_07_from_zombie.py
"""
...
ZOMBIE_SRC = (
    ROOT / "Assets/SmallScaleInt/2D Zombie Pack 1/Spritesheets/ZombieFemale6"
)
...
def build_guid_map() -> None:
    GUID_MAP.clear()
    for _png_name, mm5_guid in PNG_MM5_GUID.items():
...
def copy_pngs() -> None:
    ...
    MM7.mkdir(parents=True, exist_ok=True)
...
def bake_png_meta(png_name: str, mm7_guid: str) -> None:
    mm5_meta = MM5 / f"{png_name}.meta"
    mm7_meta = MM7 / f"{png_name}.meta"
    ...
    mm7_meta.write_text(text, encoding="utf-8")
    print(f"  meta: {png_name} -> {mm7_guid}")
...
def copy_anim_tree() -> None:
    ...
    for path in sorted(dst.rglob("*")):
        ...
        text = text.replace("MonsterModel_05", "MonsterModel_07")
        path.write_text(text, encoding="utf-8")
        if path.suffix == ".controller" and "MonsterModel_07" in path.name:
    ...
    if controller_path is not None and "MonsterModel_05" in controller_path.name:
        new_path = controller_path.with_name(
            controller_path.name.replace("MonsterModel_05", "MonsterModel_07")
        )
