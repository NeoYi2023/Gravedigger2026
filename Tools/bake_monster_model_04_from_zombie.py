#!/usr/bin/env python3
"""Bake MonsterModel_04 art from ZombieFemale1 PNGs + MonsterModel_05 clip template.

When Unity Editor is unavailable, this:
1. Copies PNGs from SmallScaleInt ZombieFemale1 (1920x1024, 128px/cell)
...
4. Removes orphaned CC-only sheets

Run from repo root: python Tools/bake_monster_model_04_from_zombie.py
"""
...
MM4 = ROOT / "Assets/Art/Characters/Monsters/MonsterModel_04"
MM5 = ROOT / "Assets/Art/Characters/Monsters/MonsterModel_05"
ZOMBIE_SRC = (
    ROOT / "Assets/SmallScaleInt/2D Zombie Pack 1/Spritesheets/ZombieFemale1"
)
...
def copy_pngs() -> None:
    ...
    MM4.mkdir(parents=True, exist_ok=True)
...
def bake_png_meta(png_name: str, mm4_guid: str) -> None:
    mm5_meta = MM5 / f"{png_name}.meta"
    mm4_meta = MM4 / f"{png_name}.meta"
    ...
    mm4_meta.write_text(text, encoding="utf-8")
    print(f"  meta: {png_name} -> {mm4_guid}")
...
def copy_anim_tree() -> None:
    ...
    for path in sorted(dst.rglob("*")):
        ...
        text = text.replace("MonsterModel_05", "MonsterModel_04")
        path.write_text(text, encoding="utf-8")
        if path.suffix == ".controller" and "MonsterModel_04" in path.name:
    ...
    if controller_path is not None and "MonsterModel_05" in controller_path.name:
        new_path = controller_path.with_name(
            controller_path.name.replace("MonsterModel_05", "MonsterModel_04")
        )
...
def remove_orphan_sheets() -> None:
    for name in ORPHAN_SHEETS:
        png = MM4 / name
        meta = MM4 / f"{name}.meta"
        if png.is_file():
            png.unlink()
            print(f"  removed {name}")
        if meta.is_file():
            meta.unlink()
            print(f"  removed {name}.meta")
...
def main() -> None:
    ...
    build_guid_map()
    remove_orphan_sheets()

    print("Copying PNGs from ZombieFemale1...")
    copy_pngs()

    print("Baking PNG metas from MonsterModel_05 template (128px, no scale)...")
    for png, mm5_guid in PNG_MM5_GUID.items():
        mm4_guid = GUID_MAP[mm5_guid]
        bake_png_meta(png, mm4_guid)
